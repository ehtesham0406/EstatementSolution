...
//using QCash.EStatement.Reports;
...
using QCash.EStatement.IFIC.Reports;
                            ...
                            if (dtCards.Rows[j]["EMAIL"].ToString().Trim() != "")
                            {
                                if (IsValid(dtCards.Rows[j]["EMAIL"].ToString().Trim()))
                                {
                                    try
                                    {
                                        ...
                                        pdfCount = pdfCount + 1;
                                        stmdt = new DataTable();
                                        //  stmdt = objProvider.ReturnData("select * from statement_BDT where IDCLIENT='" + dtCards.Rows[j]["IDCLIENT"].ToString() + "'", ref reply).Tables[0];
                                        ...
                                        if (stmdt.Rows.Count > 0)
                                        {
                                            ...
                                            //  fileName = _fiid + "_VISA_EStatement_" + dtCards.Rows[j]["idclient"].ToString() + "_" + dtCards.Rows[j]["accountno"].ToString()+ "_"  + pdfCount + ".pdf";
                                            // fileName = _fiid + "_VISA_EStatement_" + dtCards.Rows[j]["idclient"].ToString() + "_" + dtCards.Rows[j]["pan"].ToString().Substring(0, 6) + "_" + dtCards.Rows[j]["pan"].ToString().Substring(12, 4) + "_" + pdfCount + ".pdf";
                                            ...
                                            //securitySettings.UserPassword = dtCards.Rows[j]["pan"].ToString();
                                ...
                                if (IsValid(dtCards.Rows[j]["EMAIL"].ToString().Trim()))
                                {
                                    try
                                    {
                                        ...
                                        if (stmdt.Rows.Count > 0)
                                        {
                                            ...
                                            // fileName = _fiid + "_VISA_EStatement_" + dtCards.Rows[j]["idclient"].ToString() + "_" + dtCards.Rows[j]["accountno"].ToString()+"_"+pdfCount+ "_USD.pdf";
                                            string acc_no = dtCards.Rows[j]["ACCOUNTNO"].ToString();
            ...
            try
            {
                ...
                for (int f = 0; f < fi.Length; f++)
                {
                    ...
                    if (dsXML != null)
                    {
                        if (dsXML.Tables.Count > 0)
                        {
                            ...
                            for (int i = 0; i < dsXML.Tables.Count; i++)
                            {
                                if (dsXML.Tables[i].TableName == "Statement")
                                {
                                    ...
                                    objLW.logTrace(_LogPath, "EStatement.log", " : Cardholder Statement info data saved from XML..." + reply);
                                }
                                else if (dsXML.Tables[i].TableName == "Operation")
                                {
                                    reply = GetCardHolderTransactionInfo(dsXML.Tables[i]);
                                    txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : CardHolder Operation Info data Saved from XML. " + reply });
                                    objLW.logTrace(_LogPath, "EStatement.log", " : Cardholder Operation info data saved from XML..." + reply);
                                }
                                ...
                                else if (dsXML.Tables[i].TableName == "Card")
                                {
                                    ...
                                    objLW.logTrace(_LogPath, "EStatement.log", " : Cardholder Cadrd info data saved from XML..." + reply);
                                }
                                else if (dsXML.Tables[i].TableName == "BonusContrAcc")
                                {
                                    reply = GetBonusContrAccInfo(dsXML.Tables[i]);
                                    txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : CardHolder Bonus Account Info data Saved from XML. " + reply });
                                ...
                                else if (dsXML.Tables[i].TableName == "AccumIntAcc")
                                {
                                    ...
                                    txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : CardHolder Accumulate Account Info data Saved from XML. " + reply });
                        ...
                        txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : Total " + dsXML.Tables["Card"].Rows.Count.ToString() + " files found to process.." });
                        objLW.logTrace(_LogPath, "EStatement.log", " : Total " + dsXML.Tables["Card"].Rows.Count.ToString() + " files found to process..");
                    }
                    #endregion
