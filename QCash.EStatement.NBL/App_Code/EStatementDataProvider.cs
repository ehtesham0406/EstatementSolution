...
namespace StatementGenerator.App_Code
{
    public class EStatementDataProvider : IEStatement
    {
               ...
               if (ds.Tables.Count > 0) 
               {
                   if (ds.Tables[0].Rows.Count > 0)
                   {
                       ...
                       for (int i = 0; i < ds.Tables[0].Rows.Count; i++) 
                       {
                           ...
                           //objEst.MAILCC = ds.Tables[0].Rows[i]["MAILCC"].ToString();
        ...
        public EStatementList GetAllEmail(ref string reply)
        {
            EStatementList objEstList = null;
            ConStr = new ConnectionStringBuilder(1);
            objProvider = new SqlDbProvider(ConStr.ConnectionString_DBConfig);
        ...
        public StatementRegisterList GetStatementRegister(string stdate, ref string reply)
        {
            StatementRegisterList objEstList = null;
          ...
          // s="select *from (  select * from StatementRegister  union all   select * from StatementRegister_ARC) a where StatementDate='"+stdate+"' order by CAST( StatementNo as int)";
            s = "select * from StatementRegister  where StatementDate='" + stdate + "' order by CAST( StatementNo as int)";
            ...
            if (ds != null)
            {
                if (ds.Tables.Count > 0)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        objEstList = new StatementRegisterList();

                        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                        {
                            StatementRegisterInfo objEst = new StatementRegisterInfo();
                            objEst.RefNo = ds.Tables[0].Rows[i]["RefNo"].ToString();
                            //objEst.StatementNo = ds.Tables[0].Rows[i]["STATEMENTNO"].ToString();
                            objEst.ClientName = ds.Tables[0].Rows[i]["ClientName"].ToString();
                            objEst.ClientID = ds.Tables[0].Rows[i]["ClientID"].ToString();
                            objEst.Address = ds.Tables[0].Rows[i]["Address"].ToString();
                            objEst.TotalPage = ds.Tables[0].Rows[i]["TotalPage"].ToString();
                            objEst.StatementDate = ds.Tables[0].Rows[i]["StatementDate"].ToString();
                           // objEst.StartPage = ds.Tables[0].Rows[i]["StartPage"].ToString();
                           // objEst.EndPage = ds.Tables[0].Rows[i]["EndPage"].ToString();
