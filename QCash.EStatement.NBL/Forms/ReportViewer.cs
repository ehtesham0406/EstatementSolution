        ...
        void grdEmailData_DoubleClickHeader(object sender, DoubleClickHeaderEventArgs e)
        {
            if (e.Header.Column.Key == "Checks")
            {
                ...
                foreach (UltraGridRow aRow in grdEmailData.Rows)
                {
                    ...
                    ////if (aRow.Cells["Status"].Value.ToString() == "No Mail Address Found")
                    ////    aRow.Cells["Checks"].Value = true;

                   
                    ////if (aRow.Cells["Status"].Value.ToString() == "Mail is not Sent")
                    ////    aRow.Cells["Checks"].Value = true;
                                ...
                                for (int i = 0; i < objESList.Count; i++)
                                {
                                    if (objESList[i].MAILADDRESS != "")
                                    {
                                        try
                                        {
                                           ...
                                           // mail.Subject = objESList[i].MAILSUBJECT +" "+ objESList[i].PAN_NUMBER;
                                            ...
                                            //if (!string.IsNullOrEmpty(objESList[i].MAILCC))
                                            //{
                                            //    foreach (string CCAddress in objESList[i].MAILCC.ToString().Trim().Split(';'))
                                            //    {
                                            //        if (CCAddress.ToString() != "")
                                            //            if (true) mail.CC.Add(CCAddress);
                                            //    }
