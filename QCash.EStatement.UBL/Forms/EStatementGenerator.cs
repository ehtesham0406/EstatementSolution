...
using QCash.EStatement.UBL;
using CrystalDecisions.CrystalReports.Engine;
using FlexiStar.Utilities.EncryptionEngine;
using QCash.EStatement.UBL.Reports;
...
namespace StatementGenerator
{
    public partial class EStatementGenerator : Form
    {
                        ...
                        if (objSmtpList != null)
                        {
                            if (objSmtpList.Count > 0)
                            {
                                ...
                                for (int i = 0; i < objESList.Count; i++)
                                {
                                    if (objESList[i].MAILADDRESS != "")
                                    {
                                        // Create a new SmtpClient for each email to avoid rate-limiting issues
                                      //  SmtpClient SmtpServer = new SmtpClient(objSmtpList[0].Smtp_Server);
                                        try
                                        {
                                        ...
                                        //finally
                                        //{
                                            // Manually release the SmtpClient instance
                                           // SmtpServer = null;
                                       // }
        ...
        private void ProcessData()
        {
            ...
            {
                if (dia[fcount].FullName.Contains("UBL"))
                {
                    _bankName = "UBL";
                    _bankCode = "1";
