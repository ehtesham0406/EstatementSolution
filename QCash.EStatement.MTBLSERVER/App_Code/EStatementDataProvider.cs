    ...
    public class EStatementDataProvider : IEStatement
    {
        ...
        public EStatementList GetAllEStatements(string bankcode, string stdate, string status, ref string reply)
        {
          ...
          // objProvider = new SqlDbProvider(ConStr.ConnectionString_DBConfig);
           ...
           if (status == "1")
           {
               ...
               ds = dataHandler.DBOracleManupulation.RecordSetCollection(s,reply);
               //    ds = objProvider.ReturnData("(select * from email_notification where bank_code='" + bankcode + "' and status='" + status + "' and convert(date,stmdate,103)=convert(date,'" + stdate + "',103)) " +
               //" union (select * from email_notification where bank_code='" + bankcode + "' and status='" + status + "' and convert(date,stmdate,103)=convert(date,'" + stdate + "',103))", ref reply);
           }
           else if (status == "2")
               ds = dataHandler.DBOracleManupulation.RecordSetCollection("(select * from email_notification where bank_code='" + bankcode + "' and stmdate='" + stdate + "')" +
                                           " union (select * from email_notification_arc where bank_code='" + bankcode + "' and stmdate='" + stdate + "')", reply);
           else if (status == "3")
               ds = dataHandler.DBOracleManupulation.RecordSetCollection("select * from email_notification where bank_code='" + bankcode + "' and stmdate='" + stdate + "')", reply);
               ...
               if (ds.Tables.Count > 0) 
               {
                   if (ds.Tables[0].Rows.Count > 0)
                   {
                       ...
                       for (int i = 0; i < ds.Tables[0].Rows.Count; i++) 
                       {
                           ...
                           objEst.FILE_LOCATION_CLIENT = ds.Tables[0].Rows[i]["FILE_LOCATION_CLIENT"].ToString();
        ...
        public string AddEStatement(EStatementInfo objEst, ref string reply)
        {
           ...
           // objProvider = new SqlDbProvider(ConStr.ConnectionString_DBConfig);
            ...
            query = "Insert into email_notification(BANK_CODE,PAN_NUMBER,STMDATE,STM_MONTH,STM_YEAR,FILE_LOCATION,MAILADDRESS,MAILSUBJECT,MAILBODY,STATUS,FILE_LOCATION_CLIENT,CLIENT,IDCLIENT) " +
                    "Values('" + objEst.BANK_CODE + "','" + objEst.PAN_NUMBER + "','" + objEst.STMDATE + "','" +  objEst.MONTH + "','" + objEst.YEAR + "'," +
                    "'" + objEst.FILE_LOCATION + "','" + objEst.MAILADDRESS + "','" +  objEst.MAILSUBJECT + "','" + objEst.MAILBODY+ "','" + objEst.STATUS + "','" + objEst.FILE_LOCATION_CLIENT + "','" + objEst.CLIENT + "','" + objEst.IDCLIENT + "') ";
            ...
            {
                _reply = "UnSuccessfull";
            }

            //if (!_reply.Contains("Success"))
            //return _reply;
        ...
        public string UpdateEStatement(EStatementInfo objEst, ref string reply)
        {
            ...
            string query = string.Empty;
            int result;
            query = "update email_notification set  status='" + objEst.STATUS + "' where bank_code='" + objEst.BANK_CODE + "' and pan_number='" + objEst.PAN_NUMBER + "' and stmdate='" + objEst.STMDATE + "'";
