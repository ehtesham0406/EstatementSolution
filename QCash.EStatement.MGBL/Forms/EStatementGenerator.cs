...
using QCash.EStatement.MGBL;
using CrystalDecisions.CrystalReports.Engine;
using FlexiStar.Utilities.EncryptionEngine;
using QCash.EStatement.MGBL.Reports;
...
namespace StatementGenerator
{
    public partial class EStatementGenerator : Form
    {
        ...
        string vContractNo = string.Empty;
        string vPAN = string.Empty;
        string prePan = string.Empty;
        string preDoc = string.Empty;
        string preStNo = string.Empty;
        int DetailsProductSl = 0;
        ...
        void btnGenerate_Click(object sender, EventArgs e)
        {
            ...
            DataTable stDate = new DataTable();
            stDate = objProvider.ReturnData("select * from Qry_Card_Account where Curr='BDT' and stdate='" + StmDate + "'  order by CAST( Statementno as int) ASC", ref reply).Tables[0];// where Curr='BDT'
            if (stDate.Rows.Count > 0)
            {
            ...
            {
                txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : Incorrect Statement Date Input." });
                objLW.logTrace(_LogPath, "EStatement.log", System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : Incorrect Statement Date Input.");
                return;
            }
                            ...
                            if (dtCards.Rows[j]["EMAIL"].ToString().Trim() != "")
                            {
                                if (IsValid(dtCards.Rows[j]["EMAIL"].ToString().Trim()))
                                {
                                    try
                                    {
                                        ...
                                        if ( dtCards.Rows[j]["CONTRACTNO"].ToString() != vContractNo)
                                        {
                                           // vPAN = dtCards.Rows[j]["pan"].ToString();
                                            vContractNo = dtCards.Rows[j]["CONTRACTNO"].ToString();
        ...
        private void ProcessData()
        {
            ...
            {
                if (dia[fcount].FullName.Contains("MGBL"))
                {
                    _bankName = "MGBL";
                    _bankCode = "1";
        ...
        private void ProcessFolderFiles_old(string _SourcePath, string BankCode, string BankName, ref string _reply)
        {
                ...
                for (int f = 0; f < fi.Length; f++)
                {
                    ...
                    if (dsXML != null)
                    {
                        if (dsXML.Tables.Count > 0)
                        {
                            ...
                            }  // END   for (int i = 0; i < dsXML.Tables.Count; i++)
                            if (reply == "Success")
                            {
                                for (int i = 0; i < dsXML.Tables.Count; i++)
                                {
        ...
        private void ProcessFolderFiles(string _SourcePath, string BankCode, string BankName, ref string _reply)
        {
            ...
            try
            {
                ...
                // First, count all XMLs inside all ZIPs
                foreach (FileInfo file in fi)
                {
                    if (file.Extension.ToLower() == ".zip")
                    {
                        ...
                        {
                            zip.Password = "MGBL#2025!ITC";
                            foreach (Ionic.Zip.ZipEntry entry in zip.Entries)
                            {
                                if (entry.FileName.ToLower().EndsWith(".xml"))
                                    totalXmlFiles++;
                            }
                        }
                    }
                ...
                txtAnalyzer.Invoke(_addText, new object[] { DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : Total " + totalXmlFiles.ToString() + " XML files found to process in all ZIPs.." });
                objLW.logTrace(_LogPath, "EStatement.log", " : Total " + totalXmlFiles.ToString() + " XML files found to process in all ZIPs..");

                // Now process each ZIP and each XML inside it
                foreach (FileInfo file in fi)
                {
                    txtAnalyzer.Invoke(_addText, new object[] { DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : " + file.Name + " on process.." });
                    ...
                    if (file.Extension.ToLower() == ".zip")
                    {
                        using (Ionic.Zip.ZipFile zip = Ionic.Zip.ZipFile.Read(file.FullName))
                        {
                            ...
                            foreach (Ionic.Zip.ZipEntry entry in zip.Entries)
                            {
                                if (entry.FileName.ToLower().EndsWith(".xml"))
                                {
                                    string xmlFileName = entry.FileName; // Actual XML File Name Inside ZIP
                                    txtAnalyzer.Invoke(_addText, new object[] { DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : " + xmlFileName + " on process.." });
                                    ...
                                    using (MemoryStream ms = new MemoryStream())
                                    {
                                        ...
                                        dsXML.ReadXml(ms); // Read the XML

                                        if (StmDate == "")
                                            StmDate = dtpStmDate.Value.ToString("dd/MM/yyyy", CultureInfo.InvariantCulture);
                                        else StmDate = dtpStmDate.Value.ToString("dd/MM/yyyy", CultureInfo.InvariantCulture);

                                        // Find the Statement table
                                        DataTable dtStatement = dsXML.Tables["Statement"];

                                        if (dtStatement != null && dtStatement.Rows.Count > 0)
                                        {
                                            // Assuming the XML field name is 'StatementDate' (adjust if different)
                                            string xmlStatementDate = dtStatement.Rows[0]["STATEMENT_DATE"].ToString();

                                         

                                            if (xmlStatementDate != StmDate) // compare with user input
                                            {
                                                txtAnalyzer.Invoke(_addText, new object[]
                                                {
                                                    DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : XML Statement Date (" + xmlStatementDate + 
                                                    ") does not match user input (" + StmDate + "). Stopping process."
                                                });
                                                objLW.logTrace(_LogPath, "EStatement.log",
                                                    "XML Statement Date (" + xmlStatementDate + ") does not match user input (" + StmDate + "). Stopping process."
                                                );

                                                return; // stop processing further XML files
                                            }
                                        ...
                                        if (dsXML != null && dsXML.Tables.Count > 0)
                                        {
                                            ...
                                            for (int i = 0; i < dsXML.Tables.Count; i++)
                                            {
                                                ...
                                                {
                                                    GetCardHolderPersonalInfo(dsXML.Tables[i], BankName, ref reply);
                                                    txtAnalyzer.Invoke(_addText, new object[] { DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : CardHolder Personal Info data Saved from XML. " + reply });
                                                    objLW.logTrace(_LogPath, "EStatement.log", " : CardHolder Personal Info data Saved from XML. " + reply);
                                                }
                                                else if (dsXML.Tables[i].TableName == "Operation")
                                                {
                                                    reply = GetCardHolderTransactionInfo(dsXML.Tables[i]);
                                                    txtAnalyzer.Invoke(_addText, new object[] { DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : CardHolder Transaction Info data Saved from XML. " + reply });
                                                    objLW.logTrace(_LogPath, "EStatement.log", " : CardHolder Transaction Info data Saved from XML. " + reply);
                                                }
                                                else if (dsXML.Tables[i].TableName == "Account")
                                                {
                                                    reply = GetCardHolderAccountInfo(dsXML.Tables[i]);
                                                    txtAnalyzer.Invoke(_addText, new object[] { DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : CardHolder Account Info data Saved from XML. " + reply });
                                                    objLW.logTrace(_LogPath, "EStatement.log", " : CardHolder Account Info data Saved from XML. " + reply);
                                                }
                                                else if (dsXML.Tables[i].TableName == "Card")
                                                {
                                                    reply = GetCardHolderCardInfo(dsXML.Tables[i]);
                                                    txtAnalyzer.Invoke(_addText, new object[] { DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : CardHolder Card Info data Saved from XML. " + reply });
                                                    objLW.logTrace(_LogPath, "EStatement.log", " : CardHolder Card Info data Saved from XML. " + reply);
                                                }
                                                else if (dsXML.Tables[i].TableName == "BonusContrAcc")
                                                {
                                                    reply = GetBonusContrAccInfo(dsXML.Tables[i]);
                                                    txtAnalyzer.Invoke(_addText, new object[] { DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : CardHolder Reward Info data Saved from XML. " + reply });
                                                    objLW.logTrace(_LogPath, "EStatement.log", " : CardHolder Reward Info data Saved from XML. " + reply);
                                                }
                                                else if (dsXML.Tables[i].TableName == "AccumIntAcc")
                                                {
                                                    reply = GetAccumIntAcc(dsXML.Tables[i]);
                                                    txtAnalyzer.Invoke(_addText, new object[] { DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : CardHolder Suspense Account Info data Saved from XML. " + reply });
                                            ...
                                            txtAnalyzer.Invoke(_addText, new object[] { DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : Total " + dsXML.Tables["Card"].Rows.Count.ToString() + " Card record has been found to process.." });
                                            objLW.logTrace(_LogPath, "EStatement.log", " : Total " + dsXML.Tables["Card"].Rows.Count.ToString() + " Card record has been found to process..");
                                        }
                                        #endregion
                                        ...
                                        btnGenerate_Click(null, null);
                                            ...
                                            txtAnalyzer.Invoke(_addText, new object[] { DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : " + xmlFileName + " process complete.." });
                    ...
                    if (Directory.Exists(_SourcePath))
                    {
                        ...
                        {
                            txtAnalyzer.Invoke(_addText, new object[] { DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : Source Directory moving Error. Error: " + ex.Message });
                            objLW.logTrace(_LogPath, "EStatement.log", "Source Directory moving Error. " + ex.Message);
                        }
                    }
                }
            ...
            {
                _reply = ex.StackTrace;
                txtAnalyzer.Invoke(_addText, new object[] { DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : " + ex.Message });
                MsgLogWriter objLW = new MsgLogWriter();
                objLW.logTrace(_LogPath, "EStatement.log", ex.Message);
            }
            #endregion
            ...
            try
            {
                ...
                for (int k = 0; k < dtGetAccumIntAcc.Rows.Count; k++)
                {
                    ...
                    for (int j = 0; j < dtGetAccumIntAcc.Columns.Count; j++)
                    {
                        ...
                        {
                            objOp.ACCUM_INT_EBALANCE = Convert.ToDecimal(dtGetAccumIntAcc.Rows[k][j]) * (-1);
                        }
                        else if (dtGetAccumIntAcc.Columns[j].ColumnName == "ACCUM_INT_SBALANCE")
                        {
                            objOp.ACCUM_INT_SBALANCE = Convert.ToDecimal(dtGetAccumIntAcc.Rows[k][j]) * (-1);
                        }
                ...
                for (int k = 0; k < dtCard.Rows.Count; k++)
                {
                    ...
                    for (int j = 0; j < dtCard.Columns.Count; j++)
                    {
                        ...
                        else if (dtCard.Columns[j].ColumnName == "PAN")
                        {
                            ...
                            // DataTable dtCardbdt = new DataTable();
                            // dtCardbdt = objProvider.ReturnData("select SUBSTRING(MAIN_CARD, 1, 16) AS PAN  from STATEMENT where STATEMENTNO='" + objCard.STATEMENTNO + "'", ref reply).Tables[0];// where Curr='BDT'

                            // for (int M = 0; M < dtCardbdt.Rows.Count; M++)//dtCards.Rows.Count
                            // {
                            // if (dtCardbdt.Rows[M]["PAN"].ToString().Trim() != "")
                            //{
                            //  if (objCard.PAN == dtCardbdt.Rows[M]["PAN"].ToString())
                            //{
                            // objCard.CARDTYPE = "M";
                            // }
                            // else
                            // {
                            // objCard.CARDTYPE = j.ToString();
                            //}
        ...
        private void GenerateStatementInfo(DataSet dsStatement, string BankName, ref string errMsg)
        {
            ...
            objProvider.RunQuery("Delete from  BonusContrAcc where ACCOUNT_NO='7078999900800000415'");
            ...
            for (int k = 0; k < dtStatement.Rows.Count; k++)
            {
                ...
                try
                {
                    ...
                    for (int j = 0; j < dtStatement.Columns.Count; j++)
                    {
                        ...
                        {
                            objSt.SBALANCE = Convert.ToDecimal(dtStatement.Rows[k][j]) * (-1);
                        }
                        ...
                        {
                            objSt.EBALANCE = Convert.ToDecimal(dtStatement.Rows[k][j]) * (-1);
                        }
                        ...
                        if (dtOperation.Rows.Count > 0)
                        {
                            ...
                            if (dr.Length > 0)
                            {
                                    ...
                                    if (INTlist.Contains(dr[l]["D"].ToString().ToUpper()) == false)
                                    {
                                        ...
                                        if (dr[l].Table.Columns.Contains("ACURN"))
                                        {
                                            if (dr[l]["A"].ToString() == "" || dr[l]["A"].ToString() == null)
                                                objSTD.AMOUNT = Convert.ToDecimal("0.00");
                                            else
                                                objSTD.AMOUNT = Convert.ToDecimal(dr[l]["A"]) * (-1);
                                        }
                                        else objSTD.AMOUNT = Convert.ToDecimal("0.00");
                                                ...
                                                if (dr[l]["FR"].ToString().ToUpper().Contains("A 20") || dr[l]["FR"].ToString().ToUpper().Contains("A 10") || dr[l]["FR"].ToString().ToUpper().Contains("A 64") || dr[l]["FR"].ToString().ToUpper().Contains("P 14") || dr[l]["FR"].ToString().ToUpper().Contains("P 32") || dr[l]["FR"].ToString().ToUpper().Contains("P 33") || dr[l]["FR"].ToString().ToUpper().Contains("F 29") || dr[l]["FR"].ToString().ToUpper().Contains("P 13"))
                                                {
                                                    if (objSTD.OC != null)
                                                    {
                                                      objSTD.TRNDESC = dr[l]["D"].ToString().Replace("'", "''") + ": " + dr[l]["TL"].ToString().Replace("'", "''");
                                                        ...
                                                        if (objSTD.TRNDESC.Length > 60) //CHNG
                                                        {
                                ...
                                if (objSt.SUM_INTEREST != "0.00")
                                {
                                    ...
                                    objSTD.AMOUNT = Convert.ToDecimal(objSt.SUM_INTEREST);//.PadLeft(objSt.SUM_INTEREST.Length+1,'-');
                                                ...
                                                for (int x = 0; x < dtAcI.Rows.Count; x++)
                                                {
                                                    ...
                                                    if (objSTD.CONTRACTNO == dtAcI.Rows[x][1].ToString())
                                                    {
                                                        if (dtAcI.Rows[x][0].ToString() != "0.00")
                                                        {
                                                            ...
                                                            objSTD.ACURN = objSt.ACURN;
                                                            objSTD.TRNDESC = "INTEREST CHARGES";
                                                            objSTD.AMOUNT = Convert.ToDecimal(dtAcI.Rows[x][0].ToString());//.PadLeft(objSt.SUM_INTEREST.Length+1,'-');
                                                            objSTD.TRNDATE = trn_Date;
                                                            objSTD.POSTDATE = trn_Date;
                                ...
                                for (int l = 0; l < dr.Length; l++)
                                {
                                    ...
                                    if (INTlist.Contains(dr[l]["D"].ToString().ToUpper()) == false)
                                    {
                                        ...
                                        if (dr[l].Table.Columns.Contains("P"))   //Add new column from Operation 06.02.2017
                                        {
                                            ...
                                            {
                                                if (prePan != objSt.PAN && preDoc == dr[l]["DOCNO"].ToString())
                                                {
                                                    objSTD.P = prePan;
                                                    prePan = objSTD.P;
                                                }
                                        ...
                                        if (dr[l].Table.Columns.Contains("ACURN"))
                                        {
                                            if (dr[l]["A"].ToString() == "" || dr[l]["A"].ToString() == null)
                                                ...
                                                objSTD.AMOUNT = Convert.ToDecimal(dr[l]["A"].ToString()) * (-1);
                                        }
                                        else objSTD.AMOUNT = Convert.ToDecimal("0.00");
