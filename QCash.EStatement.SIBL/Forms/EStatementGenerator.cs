...
using QCash.EStatement.SIBL.Reports;
...
namespace StatementGenerator
{
    public partial class EStatementGenerator : Form
    {
        ...
        void btnGenerate_Click(object sender, EventArgs e)
        {
            ...
            dtCardbdt = objProvider.ReturnData("select * from (select distinct EMAIL,CONTRACTNO,accountno,Statementno,pan,idclient,client,StDate from Qry_Card_Account where CONTRACTNO in (select Distinct(CONTRACTNO) from statement_DUAL where Curr='BDT')) as t1  order by CAST( Statementno as int)", ref reply).Tables[0];
                                    ...
                                    try
                                    {
                                        ...
                                        if (stmdt.Rows.Count > 0)
                                        {
                                          ...
                                          //  fileName = _fiid + "_VISA_EStatement_" + dtCards.Rows[j]["idclient"].ToString() + "_" + dtCards.Rows[j]["pan"].ToString().Substring(0, 6) + "_" + dtCards.Rows[j]["pan"].ToString().Substring(12, 4) + ".pdf";
                                           ...
                                           // fileName = _fiid + "_" + dtCards.Rows[j]["pan"].ToString().Substring(0, 6) + "_" + dtCards.Rows[j]["CONTRACTNO"].ToString().Substring(0, 4) + dtCards.Rows[j]["STATEMENTNO"].ToString() + "_" + dtCards.Rows[j]["STDATE"].ToString().Replace('/', '-') + "_" + dtCards.Rows[j]["idclient"].ToString() + ".PDF";

                                           string acc_no = dtCards.Rows[j]["ACCOUNTNO"].ToString();
                                           string Pan = dtCards.Rows[j]["pan"].ToString();
                                            ...
                                            //fileName = _fiid + "_VISA_" + dtCards.Rows[j]["pan"].ToString().Substring(0, 6) + "_" + stmdt.Rows[0]["Statement_Date"].ToString().Replace('/', '-') + '_'+pdfCount + ".pdf";
                                        ...
                                        //    objst.ExportToDisk(ExportFormatType.PortableDocFormat, filePath + "\\" + fileName);
                                            ...
                                            objEst.PAN_NUMBER = dtCards.Rows[j]["pan"].ToString().Substring(0, 6)+"******"+dtCards.Rows[j]["pan"].ToString().Substring(12, 4);
                        ...
                        if (dsXML.Tables.Count > 0)
                        {
                            ...
                            for (int i = 0; i < dsXML.Tables.Count; i++)
                            {
                                ...
                                else if (dsXML.Tables[i].TableName == "AccumIntAcc")
                                {
                                    ...
                                    txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : CardHolder Suspense account data Saved from XML. " + reply });
                                    objLW.logTrace(_LogPath, "EStatement.log", " : CardHolder Suspense account data Saved from XML. " + reply);
                                } 
            ...
            try
            {
                ...
                for (int k = 0; k < dtStatement.Rows.Count; k++)
                {
                    ...
                    for (int j = 0; j < dtStatement.Columns.Count; j++)
                    {
                       ...
                       // {
                        ...
                        else if (dtStatement.Columns[j].ColumnName == "MAIN_CARD")
                        {
                            ...
                            else if (dtStatement.Rows[k][j].ToString() == "-")
                            {
                                objSt.MAIN_CARD = dtStatement.Rows[k][j].ToString().Replace("'", "''");
                            }
                            ...
                            {
                                objSt.MAIN_CARD = dtStatement.Rows[k][j].ToString().Substring(0, 6) + "******" + dtStatement.Rows[k][j].ToString().Substring(12, 4);
                            }
                        }
                    ...
                    sql = "Insert into Statement(BANK_CODE,STATEMENTNO,ADDRESS,CARD_LIST,CITY,COUNTRY,EMAIL," +
                          ...
                          "NEXT_STATEMENT_DATE,PAYMENT_DATE,REGION,STATEMENT_DATE,SEX,STREETADDRESS,TELEPHONE,TITLE,ZIP,JOBTITLE) " +
                          ...
                          "'" + objSt.TELEPHONE + "','" + objSt.TITLE + "','" + objSt.ZIP + "','" + objSt.JOBTITLE + "')";
                ...
                for (int k = 0; k < dtOperation.Rows.Count; k++)
                {
                    ...
                    for (int j = 0; j < dtOperation.Columns.Count; j++)
                    {
                        ...
                        else if (dtOperation.Columns[j].ColumnName == "P")
                        {
                            ...
                            {
                                objOp.P = dtOperation.Rows[k][j].ToString().Substring(0, 6) + "******" + dtOperation.Rows[k][j].ToString().Substring(12, 4);
                            }
                        }
                    ...
                    /*    else if (dtCard.Columns[j].ColumnName == "PAN")
                        {
                            ...
                            DT = objProvider.ReturnData("select * from CARD where STATEMENTNO='" + objCard.STATEMENTNO + "' and PAN='" + dtCard.Rows[k][j].ToString() + "' ", ref reply);
                            ...
                            {
                                if (dtCard.Rows[k][j].ToString() == "" || dtCard.Rows[k][j].ToString() == null)
                                {
                                    objCard.PAN = dtCard.Rows[k][j].ToString().Replace("'", "''");
                                }

                                else
                                {
                                    objCard.PAN = dtCard.Rows[k][j].ToString().Substring(0, 6) + "******" + dtCard.Rows[k][j].ToString().Substring(12, 4);
                                }
        ...
        private void GenerateStatementInfo(DataSet dsStatement,string BankName, ref string errMsg)
        {
            ...
            try
            {
                ...
                DataTable dtHold = dsStatement.Tables["Hold"];
                    ...
                    for (int j = 0; j < dtStatement.Columns.Count; j++)
                    {
                       ...
                       // }
                        else if (dtStatement.Columns[j].ColumnName.ToUpper() == "PAN")
                        {
                            if (dtStatement.Rows[k][j].ToString() == "" || dtStatement.Rows[k][j].ToString() == null)
                            {
                                objSt.PAN = dtStatement.Rows[k][j].ToString().Replace("'", "''");
                            }
                            ...
                            {
                                objSt.PAN = dtStatement.Rows[k][j].ToString().Substring(0, 6) + "******" + dtStatement.Rows[k][j].ToString().Substring(12, 4);
                            }
                        }
                        ...
                        if (dtOperation.Rows.Count > 0)
                        {
                            ...
                            if (dr.Length > 0)
                            {
                                ...
                                for (int l = 0; l < dr.Length; l++)
                                {
                                    ...
                                    List<string> INTlist = new List<string>() { "SUSPENSE MARK-UP PROFIT","INTEREST ON FEES & CHARGES" ,"PROFIT ON FEES & CHARGES", "INTEREST ON OTHERS","PROFIT ON OTHERS","INTEREST ON INTEREST","PROFIT ON PROFIT","INTEREST ON ATM TRANSACTION" ,"PROFIT ON ATM TRANSACTION", "INTEREST ON POS TRANSACTION","PROFIT ON POS TRANSACTION", "INTEREST ON CARD CHEQUE","PROFIT ON CARD CHEQUE",
                                        "CHARGE PROFIT FOR 0", "CHARGE PROFIT FOR 1", "CHARGE PROFIT FOR 2", "CHARGE PROFIT FOR 3", "CHARGE PROFIT FOR 4", "CHARGE PROFIT FOR 5", "CHARGE PROFIT FOR 6", "CHARGE PROFIT FOR 7", "CHARGE PROFIT FOR 8", "CHARGE PROFIT FOR 9", "CHARGE PROFIT FOR 10", "CHARGE PROFIT FOR 11", "CHARGE PROFIT FOR 0 OPERATIONS GROUP", "CHARGE PROFIT FOR 1 OPERATIONS GROUP", "CHARGE PROFIT FOR 2 OPERATIONS GROUP", "CHARGE PROFIT FOR 3 OPERATIONS GROUP", "CHARGE PROFIT FOR 4 OPERATIONS GROUP", "CHARGE PROFIT FOR 5 OPERATIONS GROUP", "CHARGE PROFIT FOR 6 OPERATIONS GROUP", "CHARGE PROFIT FOR 7 OPERATIONS GROUP", "INTEREST ON FUND TRANSFER","PROFIT ON FUND TRANSFER", "INTEREST ON BALANCE TRANSFER","PROFIT ON BALANCE TRANSFER", "INTEREST ON EMI","PROFIT ON EMI","PROFIT ON FT", "PROFIT ON BT", "INTEREST ON BANK POS TRANSACTION","PROFIT ON BANK POS TRANSACTION",  "INTEREST ON BPOS TRANSACTION","PROFIT ON BPOS TRANSACTION",
                                        "CHARGE PROFIT FOR PROFIT OPERATIONS", "CHARGE PROFIT FOR POS OPERATIONS", "CHARGE PROFIT FOR ATM OPERATIONS", "LATE PAYMENT CHARGE FOR GROUP 1", "LATE PAYMENT CHARGE FOR GROUP 2", "LATE PAYMENT CHARGE FOR GROUP 3", "CHARGE OF A DEBT FOR CREDIT OVERDRAFTING", "INTEREST ON SERVICE FEE","PROFIT ON SERVICE FEE", "INTEREST ON PREVIOUS BALANCE","PROFIT ON PREVIOUS BALANCE","REVOLVING PROFIT CHARGE" ,"CHARGE INTEREST FOR 0", "CHARGE INTEREST FOR 1", "CHARGE INTEREST FOR 2", "CHARGE INTEREST FOR 3", "CHARGE INTEREST FOR 4", "CHARGE INTEREST FOR 5", "CHARGE PROFIT FOR 6", "CHARGE INTEREST FOR 7", "CHARGE INTEREST FOR 8", "CHARGE INTEREST FOR 9", "CHARGE INTEREST FOR 10", "CHARGE INTEREST FOR 11", "CHARGE INTEREST FOR 0 OPERATIONS GROUP", "CHARGE INTEREST FOR 1 OPERATIONS GROUP", "CHARGE INTEREST FOR 2 OPERATIONS GROUP", "CHARGE INTEREST FOR 3 OPERATIONS GROUP", "CHARGE INTEREST FOR 4 OPERATIONS GROUP", "CHARGE PROFIT FOR 5 OPERATIONS GROUP", "CHARGE INTEREST FOR 6 OPERATIONS GROUP", 
                                        "CHARGE INTEREST FOR 7 OPERATIONS GROUP", "PROFIT ON FUND TRANSFER","INTEREST ON FUND TRANSFER",
                                        "CHARGE INTEREST FOR INTEREST OPERATIONS", "CHARGE INTEREST FOR POS OPERATIONS", "CHARGE INTEREST FOR ATM OPERATIONS", "LATE PAYMENT CHARGE FOR GROUP 1", "LATE PAYMENT CHARGE FOR GROUP 2", "LATE PAYMENT CHARGE FOR GROUP 3", "CHARGE OF A DEBT FOR CREDIT OVERDRAFTING", "REVOLVING INTEREST CHARGE"};
                                    ...
                                    if (INTlist.Contains(dr[l]["D"].ToString().ToUpper()) == false)
                                    {
                                        ...
                                        if (dr[l].Table.Columns.Contains("P"))   //Add new column from Operation 06.02.2017
                                        {
                                            ...
                                            {
                                                objSTD.P = dr[l]["P"].ToString().Substring(0, 6) + "******" + dr[l]["P"].ToString().Substring(12, 4);
                                                prePan   = dr[l]["P"].ToString().Substring(0, 6) + "******" + dr[l]["P"].ToString().Substring(12, 4);
                                            }
                                        }
                                        ...
                                        {
                                            objSTD.TL = dr[l]["TL"].ToString().Replace("'", "''");

                                            if (objSTD.TL.Length >= 20)
                                            {
                                                objSTD.TL = objSTD.TL.Substring(0, 20);
                                            }
                                            else
                                            {
                                                objSTD.TL = dr[l]["TL"].ToString().Replace("'", "''");
                                                ...
                                                if (dr[l]["FR"].ToString() == "" || dr[l]["FR"].ToString() == null)
                                                    if (dr[l].Table.Columns.Contains("TL"))
                                                    {
                                                       ...
                                                       // objSTD.TRNDESC = dr[l]["D"].ToString().Replace("'", "''") + " " + dr[l]["TL"].ToString().Replace("'", "''");
                                                        objSTD.TRNDESC = dr[l]["D"].ToString().Replace("'", "''") + " " + objSTD.TL;
                                        ...
                                        else
                                        {
                                            if (dr[l].Table.Columns.Contains("TL"))
                                            {
                                                if (dr[l]["FR"].ToString().ToUpper().Contains("A 10") || dr[l]["FR"].ToString().ToUpper().Contains("A 64") || dr[l]["FR"].ToString().ToUpper().Contains("P 14") || dr[l]["FR"].ToString().ToUpper().Contains("P 32") || dr[l]["FR"].ToString().ToUpper().Contains("P 33") || dr[l]["FR"].ToString().ToUpper().Contains("F 29") || dr[l]["FR"].ToString().ToUpper().Contains("P 13") || dr[l]["FR"].ToString().ToUpper().Contains("B 15"))
                                                {
                                                    ...
                                                    objSTD.TRNDESC = dr[l]["D"].ToString().Replace("'", "''") + " " + objSTD.TL.ToString().Replace("'", "''");
                                                }
                                                else
                                                {
                                                    objSTD.TRNDESC = dr[l]["D"].ToString().Replace("'", "''");
                                        ...
                                        try
                                        {
                                            if ((dr[l]["D"].ToString().ToUpper().Trim() == ("LOUNGE VISIT")))
                                            {
