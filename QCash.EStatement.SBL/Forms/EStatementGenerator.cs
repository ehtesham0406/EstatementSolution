...
using QCash.EStatement.SBL.Reports;
...
namespace StatementGenerator
{
    public partial class EStatementGenerator : Form
    {
        ...
        private void ProcessStatementBDT(DataTable dtCards)
        {
            ...
            // bool platinumChecked = platinumRB.Checked;
            // bool goldChecked = goldlRB.Checked;
            ...
            if (ds != null)
            {
                if (ds.Tables.Count > 0)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        //open cover pdf
                        //PdfSharp.Pdf.PdfDocument coverdoc = PdfReader.Open(_CoverLetterPath, PdfDocumentOpenMode.Import); 
                                ...
                                if (IsValid(dtCards.Rows[j]["EMAIL"].ToString().Trim()))
                                {
                                    try
                                    {
                                        ...
                                        if (stmdt.Rows.Count > 0)
                                        {
                                            ...
                                            // fileName = _fiid + "_VISA_EStmt_" + stmdt.Rows[0]["Statement_Date"].ToString().Replace('/', '-') + '_' + dtCards.Rows[j]["idclient"].ToString() + "_" + dtCards.Rows[j]["pan"].ToString().Substring(0, 6) + "_" + dtCards.Rows[j]["pan"].ToString().Substring(12, 4) + ".pdf";
                                            ...
                                            for (int i = 0; i < document.Pages.Count; ++i)
                                            {
                                                ...
                                                XRect layoutRectangle = new XRect(245 /*X*/ , 782 /*Y*/ , page.Width /*Width*/ , font.Height /*Height*/ );
                                            ...
                                            //     document.Pages.Insert(document.Pages.Count, coverdoc.Pages[0]);   // Cover Add
                                        ...
                                        {
                                            txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " No EStatement Has Been Generated. Please Select a template first" });
                                            objLW.logTrace(_LogPath, "EStatement.log", System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + "No EStatement Has Been Generated" + reply);
                                        }
                                ...
                                if (IsValid(dtCards.Rows[j]["EMAIL"].ToString().Trim()))
                                {
                                    try
                                    {
                                        ...
                                        if (stmdt.Rows.Count > 0)
                                        {
                                            ...
                                            if (dtCards.Rows[j]["pan"].ToString().Substring(0, 6) == "436475")
                                            {
                                                EStatementPlatinum objstPlatinum = new EStatementPlatinum();
                                                objstPlatinum.SetDataSource(stmdt);
                                                fileName = _fiid + "_VISA_EStmt_" + stmdt.Rows[0]["Statement_Date"].ToString().Replace('/', '-') + '_' + dtCards.Rows[j]["idclient"].ToString() + "_" + dtCards.Rows[j]["pan"].ToString().Substring(0, 6) + "_" + dtCards.Rows[j]["pan"].ToString().Substring(12, 4) + ".pdf";
                        ...
                        if (dsXML.Tables.Count > 0)
                        {
                            ...
                            for (int i = 0; i < dsXML.Tables.Count; i++)
                            {
                                ...
                                else if (dsXML.Tables[i].TableName == "BonusContrAcc")
                                {
                                    ...
                                    txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : CardHolder Card Reward Info data Saved from XML. " + reply });
                                ...
                                else if (dsXML.Tables[i].TableName == "AccumIntAcc")
                                {
                                    ...
                                    objLW.logTrace(_LogPath, "EStatement.log", " : CardHolder Accum Info data Saved from XML. " + reply);
                                }
                            }
                            if (reply == "Success")
                            {
                    ...
                    if (dtOperation != null && dtOperation.Columns.Contains("ACCOUNT"))
                    {
                        if (dtOperation.Rows.Count > 0)
                        {
                            ...
                            if (dr.Length > 0)
                            {
                                ...
                                {
                                    #region setting properties values
                                    List<string> INTlist = new List<string>() { "SUSPENSE MARK-UP PROFIT","INTEREST ON FEES & CHARGES", "PROFIT ON FEES & CHARGES","INTEREST ON OTHERS", "PROFIT ON OTHERS","INTEREST ON INTEREST","PROFIT ON PROFIT", "INTEREST ON ATM TRANSACTION","PROFIT ON ATM TRANSACTION", "INTEREST ON POS TRANSACTION","PROFIT ON POS TRANSACTION", "INTEREST ON CARD CHEQUE","PROFIT ON CARD CHEQUE",  "CHARGE INTEREST FOR 0","CHARGE PROFIT FOR 0", "CHARGE INTEREST FOR 1", "CHARGE PROFIT FOR 1",  "CHARGE INTEREST FOR 2","CHARGE PROFIT FOR 2",  "CHARGE INTEREST FOR 3","CHARGE PROFIT FOR 3",  "CHARGE INTEREST FOR 4","CHARGE PROFIT FOR 4",  "CHARGE INTEREST FOR 5","CHARGE PROFIT FOR 5",  "CHARGE INTEREST FOR 6","CHARGE PROFIT FOR 6",  "CHARGE INTEREST FOR 7","CHARGE PROFIT FOR 7",  "CHARGE INTEREST FOR 8","CHARGE PROFIT FOR 8", "CHARGE INTEREST FOR 9", "CHARGE PROFIT FOR 9",  "CHARGE INTEREST FOR 10","CHARGE PROFIT FOR 10",  "CHARGE INTEREST FOR 11","CHARGE PROFIT FOR 11", "CHARGE INTEREST FOR 0 OPERATIONS GROUP","CHARGE PROFIT FOR 0 OPERATIONS GROUP", "CHARGE INTEREST FOR 1 OPERATIONS GROUP","CHARGE PROFIT FOR 1 OPERATIONS GROUP", "CHARGE INTEREST FOR 2 OPERATIONS GROUP","CHARGE PROFIT FOR 2 OPERATIONS GROUP", "CHARGE INTEREST FOR 3 OPERATIONS GROUP","CHARGE PROFIT FOR 3 OPERATIONS GROUP","CHARGE INTEREST FOR 4 OPERATIONS GROUP", "CHARGE PROFIT FOR 4 OPERATIONS GROUP", "CHARGE INTEREST FOR 5 OPERATIONS GROUP","CHARGE PROFIT FOR 5 OPERATIONS GROUP", "CHARGE INTEREST FOR 6 OPERATIONS GROUP","CHARGE PROFIT FOR 6 OPERATIONS GROUP", "CHARGE INTEREST FOR 7 OPERATIONS GROUP","CHARGE PROFIT FOR 7 OPERATIONS GROUP", "PROFIT ON FUND TRANSFER", "INTEREST ON FUND TRANSFER","PROFIT ON BALANCE TRANSFER","INTEREST ON EMI", "PROFIT ON EMI","INTEREST ON FT","PROFIT ON FT", "INTEREST ON BT", "PROFIT ON BT", 
                                        "INTEREST ON BANK POS TRANSACTION", "PROFIT ON BANK POS TRANSACTION",  "INTEREST ON BPOS TRANSACTION","PROFIT ON BPOS TRANSACTION","CHARGE INTEREST FOR INTEREST OPERATIONS","CHARGE PROFIT FOR PROFIT OPERATIONS",  "CHARGE INTEREST FOR POS OPERATIONS", "CHARGE PROFIT FOR POS OPERATIONS", "CHARGE INTEREST FOR ATM OPERATIONS", "CHARGE PROFIT FOR ATM OPERATIONS", "LATE PAYMENT CHARGE FOR GROUP 1", "LATE PAYMENT CHARGE FOR GROUP 2", "LATE PAYMENT CHARGE FOR GROUP 3", "CHARGE OF A DEBT FOR CREDIT OVERDRAFTING","INTEREST ON SERVICE FEE", "PROFIT ON SERVICE FEE", "INTEREST ON PREVIOUS BALANCE","PROFIT ON PREVIOUS BALANCE","REVOLVING INTEREST CHARGE","REVOLVING PROFIT CHARGE" };
        ...
        {
            if (txtStmMsg.Text.Length >= 310)
                MessageBox.Show("Statement Message shoud be less than 310 character...");
        }

        private void goldlRB_CheckedChanged(object sender, EventArgs e)
        {
