...
namespace StatementGenerator.App_Code
{
    public class EStatementDataProvider : IEStatement
    {
                   ...
                   if (ds.Tables[0].Rows.Count > 0)
                   {
                       ...
                       for (int i = 0; i < ds.Tables[0].Rows.Count; i++) 
                       {
                          ...
                          // objEst.MONTH = ds.Tables[0].Rows[i]["STM_MONTH"].ToString();
                           ...
                           string card = ds.Tables[0].Rows[i]["PAN_NUMBER"].ToString();
                           objEst.PAN_NUMBER = card.Substring(0, 6) + "******" + card.Substring(12, 4);
                           ...
                           if (objEst.STATUS == "1")
                           {
                               objEst.STATUS = "The statement has been successfully generated.";
                           }
                           else if (objEst.STATUS == "0")
                           {
                               objEst.STATUS = "The mail has been sent successfully.";
                           }
                           else if (objEst.STATUS == "2")
                           {
                               objEst.STATUS = "The mail could not be sent";
                           }
                           else if (objEst.STATUS == "8")
                           {
                               objEst.STATUS = "Invalid or No Mail Address Found.";
                           }
        ...
        public bool AlreadyProcessedEStatements(string bankcode, string stdate, string pan, ref string reply)
        {
            ...
            DataSet ds = new DataSet();
            ds = objProvider.ReturnData("select * from email_notification where bank_code='" + bankcode + "' and idclient='" + pan + "' and stmdate='" + stdate + "' ", ref reply);
            if (ds != null)
            {
                if (ds.Tables.Count > 0)
                {
        ...
        public string AddEStatement(EStatementInfo objEst, ref string reply)
        {
            ...
            _reply = objProvider.RunQuery("insert into email_notification values('" + objEst.BANK_CODE + "','" + objEst.PAN_NUMBER + "','" + objEst.STMDATE + "','" + objEst.MONTH + "','" + objEst.YEAR + "','" + objEst.FILE_LOCATION + "','" + objEst.MAILADDRESS + "','" + objEst.MAILSUBJECT + "','" + objEst.MAILBODY + "','" + objEst.STATUS + "','" + objEst .IDCLIENT+ "')");
            return _reply;
        }
        ...
        public string ArchiveEStatementCardData(ref string reply)
        {
            int qStatus = 0;
            string _reply = string.Empty;
            try
            {
                ...
                qStatus = objProvider.ExecuteNonQuery("sp_ArchieveCardData", null);
