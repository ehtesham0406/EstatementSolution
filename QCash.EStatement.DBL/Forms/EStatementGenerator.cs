...
using QCash.EStatement.DBL.Reports;
    ...
    public partial class EStatementGenerator : Form
    {
                        ...
                        if (objSmtpList != null)
                        {
                            if (objSmtpList.Count > 0)
                            {
                                ...
                                for (int i = 0; i < objESList.Count; i++)
                                {
                                    ...
                                    {
                                        txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : " + " Invalid or No Mail Address Found to send the Estatement " + objESList[i].FILE_LOCATION + " " +" PAN : " + objESList[i].PAN_NUMBER + " and Email : " + email }); ;
                                        objLW.logTrace(_LogPath, "EStatement.log", System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : Invalid or No Mail Address Found to send the Estatement " + objESList[i].FILE_LOCATION + " " + " PAN : " + objESList[i].PAN_NUMBER + " and Email : " + email);

                                        objESList[i].STATUS = "8";   //  Invalid or No Mail Address Found
                                        EStatementManager.Instance().UpdateEStatement(objESList[i], ref reply);
                                    }
                                }
                                    ...
                                    try
                                    {
                                        ...
                                        if ((dtCards.Rows[j]["pan"].ToString()) != vPAN)
                                        {
                                            ...
                                            if (stmdt.Rows.Count > 0)
                                            {
                                                ...
                                                string[] drdate = stmdt.Rows[0]["STATEMENT_DATE"].ToString().Split(new char[] { '/', '-' });
                ...
                if (ds.Tables.Count > 0)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        for (int j = 0; j < dtCards.Rows.Count; j++)//dtCards.Rows.Count
                        {
                            ...
                            //else
                            //{
                            //    txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + ": WARNING! No Email Address present. \n : Estatement Generatede but email will not be sent for Card# " + dtCards.Rows[j]["PAN"].ToString().Substring(0, 6) + "******" + dtCards.Rows[j]["PAN"].ToString().Substring(12, 4) });
                            //    objLW.logTrace(_LogPath, "EStatement.log", System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + ": WARNING! No Email Address present. \n : Estatement has not been created for Card# " + dtCards.Rows[j]["PAN"].ToString().Substring(0, 6) + "******" + dtCards.Rows[j]["PAN"].ToString().Substring(12, 4));
                            //    try
                            //    {
                            //        pdfCount = pdfCount + 1;
                            //        stmdt = new DataTable();
                ...
                for (int k = 0; k < dtStatement.Rows.Count; k++)
                {
                    ...
                    for (int j = 0; j < dtStatement.Columns.Count; j++)
                    {
                        ...
                        else if (dtStatement.Columns[j].ColumnName == "PromotionalText")
                        {
                            ...
                            if (pindex > 0)
                            {
                                objSt.PPROMOTIONALTEXT = objSt.PTEXT.Substring(0, pindex).ToString().Replace("'", "''");
                            }
                ...
                for (int k = 0; k < dtCard.Rows.Count; k++)
                {
                    ...
                    for (int j = 0; j < dtCard.Columns.Count; j++)
                    {
                        ...
                        else if (dtCard.Columns[j].ColumnName == "PAN")
                        {
                            ...
                            objCard.BIN = dtCard.Rows[k][j].ToString().Substring(0, 6);
                        }
                    ...
                    sql = "Insert into Card(STATEMENTNO,PAN,MBR,CLIENTNAME,SLNO,BIN)" +
                       " Values('" + objCard.STATEMENTNO + "','" + objCard.PAN + "','" + objCard.MBR + "','" + objCard.CLIENTNAME + "','" + @vSL + "','" + objCard.BIN + "')";
                ...
                try
                {
                    ...
                    for (int j = 0; j < dtStatement.Columns.Count; j++)
                    {
                        ...
                        else if (dtStatement.Columns[j].ColumnName.ToUpper() == "PAN")
                        {
                            ...
                            {
                                txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : Card Not found for the Contract " + objSt.CONTRACTNO });
                                MsgLogWriter objLW = new MsgLogWriter();
                                    ...
                                    if (INTlist.Contains(dr[l]["D"].ToString().ToUpper()) == false)
                                    {
                                        ...
                                        else
                                        {
                                            if (dr[l].Table.Columns.Contains("TL"))
                                            {
                                                ...
                                                if (dr[l]["FR"].ToString().ToUpper().Contains("P 10") || dr[l]["FR"].ToString().ToUpper().Contains("A 10") || dr[l]["FR"].ToString().ToUpper().Contains("P 33") || dr[l]["FR"].ToString().ToUpper().Contains("P 32") || dr[l]["FR"].ToString().ToUpper().Contains("P 14") || dr[l]["FR"].ToString().ToUpper().Contains("F 29") || dr[l]["FR"].ToString().ToUpper().Contains("A 64"))
                                                {
                                                    objSTD.TRNDESC = dr[l]["D"].ToString().Replace("'", "''") + " " + dr[l]["TL"].ToString().Replace("'", "''");
                                        ...
                                        var Entrylist = new List<String>() { "Credit cash deposit", "Credit acct", "CASH BACK[BDT]", "CREDIT ADJUSTMENT", "ATM INTEREST (REVERSE)", "CARD FEE(REVERSE)", "CASH ADVANCE FEE(REVERSE)", "LATE PAYMENT FEE(REVERSE)", "OVER LIMIT FEE(REVERSE)", "PIN FEE(REVERSE)", "POS INTEREST(REVERSE)", "CARD REPLACEMENT FEE(REVERSE)", "ATM INTEREST", "BAL_TRNS", "DEBIT ADJUSTMENT", "FUND TRANSFER", "INTEREST CHARGE", "POS INTEREST", "CASH DEPOSIT (REVERSE)", "SALES_SLIP_RET_FEE", "STATEMENT REPRINT FEE", "VAT", "MANUAL_TXN[POS]", "MANUAL_TXN[POS-R]", "REVERSAL-POS PURCHASE" };
                                        ...
                                        if (dr[l].Table.Columns.Contains("FR"))
                                        {
                                            if (dr[l]["FR"].ToString() != "" || dr[l]["FR"].ToString() != null)
                                            {
                                                if (dr[l]["FR"].ToString().ToUpper() == "INSTALOAN INSTALLMENT" && dr[l]["D"].ToString().ToUpper() == "SWIPE IT INSTALLMENT")
                                                {
                                                    objSTD.TRNDESC = dr[l]["FR"].ToString().Replace("'", "''");
                                                }
                                            }
                                        ...
                                        if (dr[l].Table.Columns.Contains("APPROVAL"))
                                        {
                                            ...
                                            {
                                                objSTD.TRNDATE = getNumberFormat(dr[l]["OD"].ToString());
                                            }
                                        }
                                        ...
                                        #region LATE PAYMENT FEE


                                        if (dr[l]["D"].ToString().ToUpper().Trim() == "LATE PAYMENT FEE (INCL. VAT)")
                                        {
                                            DataTable dtt = new DataTable();
                                            dtt = objProvider.ReturnData("SELECT *  FROM  CARD WHERE STATEMENTNO='" + objSt.STATEMENTNO + "' AND BIN = '" + "479941" + "'", ref reply).Tables[0];// where Curr='BDT'

                                            if (dtt.Rows.Count > 0)
                                            {
                                                objSTD.TRNDESC = "SERVICE EXTENSION FEE";
                                            }
                                            else
                                            {
                                                objSTD.TRNDESC = dr[l]["D"].ToString().Replace("'", "''");
        ...
        private bool IsValid_old(string emailaddress)
        {
            try
            {
                ...
                System.Net.Mail.MailAddress ma = new MailAddress(emailaddress);
                return true;
            }
