...
using QCash.EStatement.MMBL.Reports;
...
namespace StatementGenerator
{
    public partial class EStatementGenerator : Form
    {
                                ...
                                if (IsValid(dtCards.Rows[j]["EMAIL"].ToString().Trim()))
                                {
                                    try
                                    {
                                        ...
                                        if (stmdt.Rows.Count > 0)
                                        {
                                            ...
                                            EStatement objst = new EStatement();
                                            objst.SetDataSource(stmdt);
                                            // string _Contract_Type = dtCards.Rows[j]["CONTRACTNO"].ToString().Substring(0, 4);
                                            //  string _maskCon = _CONTRACT_NO.Substring(_CONTRACT_NO.Length - 4);
                                            //    fileName = _fiid + "_" + dtCards.Rows[j]["pan"].ToString().Substring(0, 6) + "_" + dtCards.Rows[j]["CONTRACTNO"].ToString().Substring(0, 4) +"_"+ dtCards.Rows[j]["idclient"].ToString() + ".pdf";
                                            ...
                                            //fileName = _fiid + "_VISA_EStatement_" + dtCards.Rows[j]["idclient"].ToString() _+ "_" + dtCards.Rows[j]["pan"].ToString().Substring(0, 6) + "_" + dtCards.Rows[j]["pan"].ToString().Substring(12, 4) + ".pdf";
                                            ...
                                            for (int i = 0; i < document.Pages.Count; ++i)
                                            {
                                                ...
                                                XRect layoutRectangle = new XRect(265 /*X*/ , 757 /*Y*/ , page.Width /*Width*/ , font.Height /*Height*/ );
        ...
        private void ProcessData()
        {
            ...
            {
                if (dia[fcount].FullName.Contains("kktBgfdgdfsL"))
                {
                    _bankName = "MBL";
                    _bankCode = "3";
                ...
                else if (dia[fcount].FullName.Contains("MMBL"))
                {
                    _bankName = "MMBL";
                    _bankCode = "18";
                    _XMLSourcePath = dia[fcount].FullName;
                        ...
                        if (dsXML.Tables.Count > 0)
                        {
                            ...
                            for (int i = 0; i < dsXML.Tables.Count; i++)
                            {
                                ...
                                else if (dsXML.Tables[i].TableName == "AccumIntAcc")
                                {
                                    ...
                                    txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : CardHolder Accum Info data Saved from XML. " + reply });
                ...
                for (int k = 0; k < dtStatement.Rows.Count; k++)
                {
                    ...
                    for (int j = 0; j < dtStatement.Columns.Count; j++)
                    {
                        ...
                        else if (dtStatement.Columns[j].ColumnName == "Address")
                        {
                            ...
                            //string tempo = regex.Replace(objSt.ADDRESS, " ");
                            objSt.ADDRESS = regex.Replace(dtStatement.Rows[k][j].ToString().Replace("'", ""), " ");*/
        ...
        private string GetCardHolderCardInfo(DataTable dtCard)
        {
            ...
            //MsgLogWriter objLW1 = new MsgLogWriter();
                    ...
                    if (dtOperation != null && dtOperation.Columns.Contains("ACCOUNT"))
                    {
                        if (dtOperation.Rows.Count > 0)
                        {
                            ...
                            if (dr.Length > 0)
                            {
                                ...
                                {
                                    #region setting properties values
                                    List<string> INTlist = new List<string>() { "INTEREST ON FEES & CHARGES","INTEREST ON OTHERS", "INTEREST ON INTEREST", "INTEREST ON ATM TRANSACTION", "INTEREST ON POS TRANSACTION", "INTEREST ON CARD CHEQUE", "CHARGE INTEREST FOR 1", "CHARGE INTEREST FOR 2", "CHARGE INTEREST FOR 3", "CHARGE INTEREST FOR 4", "CHARGE INTEREST FOR 5", "CHARGE INTEREST FOR 6", "CHARGE INTEREST FOR 7", "CHARGE INTEREST FOR 8", "CHARGE INTEREST FOR 9", "CHARGE INTEREST FOR 10", "CHARGE INTEREST FOR 11", "CHARGE INTEREST FOR 0 OPERATIONS GROUP", "CHARGE INTEREST FOR 1 OPERATIONS GROUP", "CHARGE INTEREST FOR 2 OPERATIONS GROUP", "CHARGE INTEREST FOR 3 OPERATIONS GROUP", "CHARGE INTEREST FOR 4 OPERATIONS GROUP", "CHARGE INTEREST FOR 5 OPERATIONS GROUP", "CHARGE INTEREST FOR 6 OPERATIONS GROUP", "CHARGE INTEREST FOR 7 OPERATIONS GROUP", "INTEREST ON FUND TRANSFER", "INTEREST ON BALANCE TRANSFER", "INTEREST ON EMI", "INTEREST ON FT", "INTEREST ON BT", "INTEREST ON BANK POS TRANSACTION",
