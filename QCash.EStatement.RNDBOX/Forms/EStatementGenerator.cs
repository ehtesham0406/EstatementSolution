...
using QCash.EStatement.RNDBOX.Reports;
using QCash.EStatement.RNDBOX.App_Code;
...
namespace StatementGenerator
{
    public partial class EStatementGenerator : Form
    {
        ...
        void btnGenerate_Click(object sender, EventArgs e)
        {
            ...
            {
                txtAnalyzer.Invoke(_addText, new object[] { "\n" + System.DateTime.Now.ToString("dd.MM.yyyy hh:mm:ss") + " : Processing Estatement BDT." });
                objLW.logTrace(_LogPath, "EStatement.log", System.DateTime.Now.ToString("dd.MM.yyyy hh:mm:ss") + " : Processing Estatement BDT.");
                                ...
                                //    {
                                        if (dtCards.Rows[j]["LASTBAL"].ToString().Trim() != "0.00")
                                        {
                                            try
                                            {
                                                ...
                                                stmdt = objProvider.ReturnData("select * from statement_BDT where CONTRACTNO='" + dtCards.Rows[j]["CONTRACTNO"].ToString() + "'", ref reply).Tables[0];
                                    ...
                                    //else
                                    //{
                                    //    txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("dd.MM.yyyy hh:mm:ss") + " : Outstanding is Zero. " + dtCards.Rows[j]["EMAIL"].ToString().Trim() + " \n : Estatement has not been created for Card# " + dtCards.Rows[j]["PAN"].ToString().Substring(0, 6) + "******" + dtCards.Rows[j]["PAN"].ToString().Substring(12, 4) });
                                    //    objLW.logTrace(_LogPath, "EStatement.log", System.DateTime.Now.ToString("dd.MM.yyyy hh:mm:ss") + " : Outstanding is Zero. " + dtCards.Rows[j]["EMAIL"].ToString().Trim() + " \n : Estatement has not been created for Card# " + dtCards.Rows[j]["PAN"].ToString().Substring(0, 6) + "******" + dtCards.Rows[j]["PAN"].ToString().Substring(12, 4));
        ...
        private void ProcessData()
        {
            ...
            for (int fcount = 0; fcount < dia.Length; fcount++)
            {
                ...
                else if (dia[fcount].FullName.Contains("RNDBOX"))
                {
                    _bankName = "RNDBOX";
                    _bankCode = "1";
                    _XMLSourcePath = dia[fcount].FullName;
            ...
            try
            {
                ...
                for (int f = 0; f < fi.Length; f++)
                {
                    ...
                    if (dsXML != null)
                    {
                        if (dsXML.Tables.Count > 0)
                        {
                            ...
                            for (int i = 0; i < dsXML.Tables.Count; i++)
                            {
                                if (dsXML.Tables[i].TableName == "Statement")
                                {
                                    ...
                                    txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("dd.MM.yyyy hh:mm:ss") + " : CardHolder statement info Saved from XML..." + reply });
                                ...
                                else if (dsXML.Tables[i].TableName == "Operation")
                                {
                                    ...
                                    txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("dd.MM.yyyy hh:mm:ss") + " : CardHolder Operation info Saved from XML..." + reply });
                                ...
                                else if (dsXML.Tables[i].TableName == "Account")
                                {
                                    ...
                                    txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("dd.MM.yyyy hh:mm:ss") + " : CardHolder Account info Saved from XML..." + reply });
                                ...
                                else if (dsXML.Tables[i].TableName == "Card")
                                {
                                    ...
                                    txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("dd.MM.yyyy hh:mm:ss") + " : CardHolder Card info Saved from XML..." + reply });
                        ...
                        txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("dd.MM.yyyy hh:mm:ss") + " : Total " + dsXML.Tables["Card"].Rows.Count.ToString() + " record found to process.." });
                        objLW.logTrace(_LogPath, "EStatement.log", " : Total " + dsXML.Tables["Card"].Rows.Count.ToString() + " record found to process..");
                    }
                    #endregion

                    txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("dd.MM.yyyy hh:mm:ss") + " : " + fi[f].Name + " data capture complete.." });
                ...
                if (Directory.Exists(_SourcePath))
                {
                    ...
                    {
                        txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("dd.MM.yyyy hh:mm:ss") + " : Source Directory moving Error. " + ex.Message });
        ...
        private void GenerateStatementInfo(DataSet dsStatement,string BankName,string filename, ref string errMsg)
        {
            ...
            {
                txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("dd.MM.yyyy hh:mm:ss") + " : " + ex.Message +", Problem in file: "+ filename});
                MsgLogWriter objLW = new MsgLogWriter();
                objLW.logTrace(_LogPath, "EStatement.log", ex.Message);
                errMsg = ex.StackTrace;
            }
                    ...
                    if (dtOperation != null && dtOperation.Columns.Contains("ACCOUNT"))
                    {
                        if (dtOperation.Rows.Count > 0)
                        {
                            ...
                            if (dr.Length > 0)
                            {
                                ...
                                {
                                    #region setting properties values
                                    List<string> INTlist = new List<string>() { "INTEREST ON FEES & CHARGES", "INTEREST ON INTEREST", "INTEREST CHARGE", "INTEREST ON ATM TRANSACTION", "INTEREST ON POS TRANSACTION", "INTEREST ON CARD CHEQUE", "CHARGE INTEREST FOR 0", "CHARGE INTEREST FOR 1", "CHARGE INTEREST FOR 2", "CHARGE INTEREST FOR 3", "CHARGE INTEREST FOR 4", "CHARGE INTEREST FOR 5", "CHARGE INTEREST FOR 6", "CHARGE INTEREST FOR 7", "CHARGE INTEREST FOR 8", "CHARGE INTEREST FOR 9", "CHARGE INTEREST FOR 10", "CHARGE INTEREST FOR 11", "CHARGE INTEREST FOR 0 OPERATIONS GROUP", "CHARGE INTEREST FOR 1 OPERATIONS GROUP", "CHARGE INTEREST FOR 2 OPERATIONS GROUP", "CHARGE INTEREST FOR 3 OPERATIONS GROUP", "CHARGE INTEREST FOR 4 OPERATIONS GROUP", "CHARGE INTEREST FOR 5 OPERATIONS GROUP", "CHARGE INTEREST FOR 6 OPERATIONS GROUP", "CHARGE INTEREST FOR 7 OPERATIONS GROUP", "INTEREST ON FUND TRANSFER", "INTEREST ON BALANCE TRANSFER", "INTEREST ON EMI","INTEREST ON MONTHLY EMI", "INTEREST ON FT", "INTEREST ON BT", "INTEREST ON BANK POS TRANSACTION", "REVOLVING INTEREST CHARGE" };
                                    if (INTlist.Contains(dr[l]["D"].ToString().ToUpper()) == false)
                                    {
                                        ...
                                        #region Rest of Txn
                                        else
                                            {
                                                if (dr[l].Table.Columns.Contains("TL"))
                                                {
                                                    if (dr[l]["FR"].ToString().ToUpper().Contains("P 10") || dr[l]["FR"].ToString().ToUpper().Contains("A 10") || dr[l]["FR"].ToString().ToUpper().Contains("P 33"))
                                                    {
                                                        objSTD.TRNDESC = dr[l]["D"].ToString().Replace("'", "''") + " " + dr[l]["TL"].ToString().Replace("'", "''");
                                ...
                                if (objSt.SUM_INTEREST != "0.00")
                                {
                                    ...
                                    sql = "Insert into STATEMENT_DETAILS(STATEMENTID,CONTRACTNO,IDCLIENT,PAN,ACCOUNTNO,STATEMENTNO,TRNDATE,POSTDATE,TRNDESC,ACURN,AMOUNT,APPROVAL,AMOUNTSIGN,FR,SERIALNO,DE)" +
                                            " VALUES('" + objSTD.STATEMENTID + "','" + objSTD.CONTRACTNO + "','" + objSTD.IDCLIENT + "','" + objSTD.PAN + "','" + objSTD.ACCOUNTNO + "','" + objSTD.STATEMENTNO + "','" + objSTD.TRNDATE + "'," +
                                            "'" + objSTD.POSTDATE + "','" + objSTD.TRNDESC + "','" + objSTD.ACURN + "','" + objSTD.AMOUNT + "','" + objSTD.APPROVAL + "','" + objSTD.AMOUNTSIGN + "','" + objSTD.FR + "','" + objSTD.SERIALNO + "','" + objSTD.DE + "')";
                                            ...
                                            if (dsAcI.Tables[0].Rows.Count > 0)
                                            {
                                                ...
                                                for (int x = 0; x < dtAcI.Rows.Count; x++)
                                                {
                                                    ...
                                                    if (objSTD01.CONTRACTNO.Trim() == dtAcI.Rows[x][1].ToString().Trim())
                                                    {
                                                        ...
                                                        " ACURN= '" + objSTD01.ACURN + "',AMOUNT= '" + tempTotalIntAmt + "',APPROVAL= '" + objSTD01.APPROVAL + "',AMOUNTSIGN= '" + objSTD01.AMOUNTSIGN + "'" +
                    ...
                    if (dtOperation != null && dtOperation.Columns.Contains("ACCOUNT"))
                    {
                        if (dtOperation.Rows.Count > 0)
                        {
                            ...
                            if (dr.Length > 0)
                            {
                                ...
                                {
                                    #region setting properties values
                                    List<string> INTlist = new List<string>() { "INTEREST ON FEES & CHARGES", "INTEREST CHARGE", "INTEREST ON INTEREST", "INTEREST ON ATM TRANSACTION", "INTEREST ON POS TRANSACTION", "INTEREST ON CARD CHEQUE", "CHARGE INTEREST FOR 0", "CHARGE INTEREST FOR 1", "CHARGE INTEREST FOR 2", "CHARGE INTEREST FOR 3", "CHARGE INTEREST FOR 4", "CHARGE INTEREST FOR 5", "CHARGE INTEREST FOR 6", "CHARGE INTEREST FOR 7", "CHARGE INTEREST FOR 8", "CHARGE INTEREST FOR 9", "CHARGE INTEREST FOR 10", "CHARGE INTEREST FOR 11", "CHARGE INTEREST FOR 0 OPERATIONS GROUP", "CHARGE INTEREST FOR 1 OPERATIONS GROUP", "CHARGE INTEREST FOR 2 OPERATIONS GROUP", "CHARGE INTEREST FOR 3 OPERATIONS GROUP", "CHARGE INTEREST FOR 4 OPERATIONS GROUP", "CHARGE INTEREST FOR 5 OPERATIONS GROUP", "CHARGE INTEREST FOR 6 OPERATIONS GROUP", "CHARGE INTEREST FOR 7 OPERATIONS GROUP", "INTEREST ON FUND TRANSFER", "INTEREST ON BALANCE TRANSFER", "INTEREST ON EMI","INTEREST ON MONTHLY EMI", "INTEREST ON FT", "INTEREST ON BT", "INTEREST ON BANK POS TRANSACTION", "REVOLVING INTEREST CHARGE" };
                                    if (INTlist.Contains(dr[l]["D"].ToString().ToUpper()) == false)
                                    {
                                        StatementDetails objSTD = new StatementDetails();
                                        objSTD.STATEMENTID = objSt.STATEMENTID;
