...
namespace StatementGenerator
{
    public partial class ReportViewer : Form
    {
        ...
        void grdEmailData_InitializeLayout(object sender, Infragistics.Win.UltraWinGrid.InitializeLayoutEventArgs e)
        {
            ...
            e.Layout.Bands[0].Columns["STARTDATE"].Width = 70;
            e.Layout.Bands[0].Columns["STARTDATE"].CellActivation = Activation.ActivateOnly;
            //
            e.Layout.Bands[0].Columns["ENDDATE"].Header.Caption = "End \nDate";
            e.Layout.Bands[0].Columns["ENDDATE"].Width = 70;
            e.Layout.Bands[0].Columns["ENDDATE"].CellActivation = Activation.ActivateOnly;
            //
            e.Layout.Bands[0].Columns["IDCLIENT"].Header.Caption = "Id \nClient";
            e.Layout.Bands[0].Columns["IDCLIENT"].Width = 50;
            e.Layout.Bands[0].Columns["IDCLIENT"].CellActivation = Activation.ActivateOnly;
            //
            e.Layout.Bands[0].Columns["PAN"].Header.Caption = "Pan";
            e.Layout.Bands[0].Columns["PAN"].Width = 120;
            e.Layout.Bands[0].Columns["PAN"].CellActivation = Activation.ActivateOnly;
            //
        ...
        void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (StartDate == "" && EndDate == "")
                {
                    ...
                    StartDate = dtpStartDate.Value.ToString("dd/MM/yyyy");
                    EndDate = dtpEndDate.Value.ToString("dd/MM/yyyy");
                }
                ...
                EStatementList objESList = EStatementManager.Instance().GetAllEStatements(_fiid, StartDate, EndDate, "2", ref reply);
                if (objESList != null)
                {
                                ...
                                for (int i = 0; i < objESList.Count; i++)
                                {
                                    if (objESList[i].MAILADDRESS != "")
                                    {
                                        try
                                        {
                                            ...
                                            mail.Subject = objESList[i].MAILSUBJECT + " " + objESList[i].PAN.ToString().Substring(0, 6) + "*********" + objESList[i].PAN.ToString().Substring(15, 1);
