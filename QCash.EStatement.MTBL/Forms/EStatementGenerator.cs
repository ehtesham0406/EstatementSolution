...
using QCash.EStatement.MTBL.Reports;
...
namespace StatementGenerator
{
    public partial class EStatementGenerator : Form
    {
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        for (int j = 0; j < dtCards.Rows.Count; j++)//dtCards.Rows.Count
                        {
                                    ...
                                    try
                                    {
                                        pdfCount = pdfCount + 1;
                                        stmdt = new DataTable();
                                        stmdt = objProvider.ReturnData("select * from Statement_DUAL where CONTRACTNO='" + dtCards.Rows[j]["CONTRACTNO"].ToString() + "' ORDER BY [AutoID]", ref reply).Tables[0];
        ...
        void ReportViewer_Load(object sender, EventArgs e)
        {
            ...
            _serverName = ConfigurationManager.AppSettings[10].ToString();
            _databaseName = ConfigurationManager.AppSettings[11].ToString();
            _userId = ConfigurationManager.AppSettings[12].ToString();
            _Password = ConfigurationManager.AppSettings[13].ToString();
        ...
        private string CalcuateBalance()
        {
            ...
            try
            {
                //Data Source=(local);user id=rabby;pwd=123;Initial Catalog=EStatementMasked_MTBL; Connection Timeout = 60000
                string str = "server='"+_serverName+"'; Initial Catalog='"+_databaseName+"'; User ID='"+_userId+"'; Password='"+_Password+"'";  // LOCAL
              //  string str = "server=MDC-CESO; Initial Catalog=EStatementMasked3; User ID=sa; Password=DB123#456";
                SqlConnection cn = new SqlConnection(str);
                SqlCommand cmd = new SqlCommand("UPDATE_LIMIT", cn);
            ...
            try
            {
                ...
                for (int k = 0; k < dtStatement.Rows.Count; k++)
                {
                    ...
                    for (int j = 0; j < dtStatement.Columns.Count; j++)
                    {
                        ...
                        //else if (dtStatement.Columns[j].ColumnName == "PROMO_TEXT")
                        //{
                    ...
                    sql = "Insert into Statement(BANK_CODE,STATEMENTNO,ADDRESS,CARD_LIST,CITY,COUNTRY,EMAIL," +
                          ...
                          "NEXT_STATEMENT_DATE,PAYMENT_DATE,REGION,STATEMENT_DATE,SEX,STREETADDRESS,TELEPHONE,TITLE,ZIP,PROMOTIONALTEXT) " +
                          ...
                          "'" + objSt.TELEPHONE + "','" + objSt.TITLE + "','" + objSt.ZIP + "','" + objSt.PROMOTIONALTEXT + "')";
            ...
            try
            {
                ...
                for (int k = 0; k < dtCard.Rows.Count; k++)
                {
                   ...
                   // objCardList.Add(objCard);
                    ...
                    //if (reply.Contains("Success"))
                    //{
                    //    txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : Message: DATA is unable to insert in Database for " + objCard.STATEMENTNO });
                    //    continue;
                    //}
                    //else
                    //{
                        ...
                        //{
                          ...
                          //  string[] lines = value.Split(new char[] { '|' });
                           ...
                           // if (!string.IsNullOrEmpty(lines[1]))
                            //{
                               // objSt.MMFBDTSBALANCE = lines[1].Trim();
                           // }
                    ...
                    if (dtOperation != null && dtOperation.Columns.Contains("ACCOUNT"))
                    {
                        if (dtOperation.Rows.Count > 0)
                        {
                            ...
                            if (dr.Length > 0)
                            {
                                ...
                                {
                                    #region setting properties values
                                    List<string> INTlist = new List<string>() { "MARK UP PROFIT(R)", "MARK UP PROFIT (R)", "MARK UP PROFIT (R) (REVERSE)", "INTEREST ON FEES & CHARGES", "INTEREST ON INTEREST","INTEREST ON ATM TRANSACTION","INTEREST ON ATM","INTEREST ON POS TRANSACTION","INTEREST ON POS", "INTEREST ON CARD CHEQUE","INTEREST ON FUND TRANSFER","INTEREST ON BALANCE TRANSFER","INTEREST ON EMI","INTEREST ON BANK POS TRANSACTION","CHARGE INTEREST FOR 1","CHARGE INTEREST FOR 2","CHARGE INTEREST FOR 3","CHARGE INTEREST FOR 4","CHARGE INTEREST FOR 5","CHARGE INTEREST FOR 6","CHARGE INTEREST FOR 7","CHARGE INTEREST FOR 8",
                                     "CHARGE INTEREST FOR 9","CHARGE INTEREST FOR 10","CHARGE INTEREST FOR 11","CHARGE INTEREST FOR 12","CHARGE INTEREST FOR 13","CHARGE INTEREST FOR 0 OPERATIONS GROUP","CHARGE INTEREST FOR 1 OPERATIONS GROUP","CHARGE INTEREST FOR 2 OPERATIONS GROUP","CHARGE INTEREST FOR 3 OPERATIONS GROUP","CHARGE INTEREST FOR 4 OPERATIONS GROUP","CHARGE INTEREST FOR 5 OPERATIONS GROUP","LATE PAYMENT CHARGE FOR GROUP 1","LATE PAYMENT CHARGE FOR GROUP 2","LATE PAYMENT CHARGE FOR GROUP 3","CHARGE OF A DEBT FOR CREDIT OVERDRAFTING" ,"INTEREST ON BANK POS TRANSACTION","INTEREST ON BPOS TRANSACTION","INTEREST ON SERVICE FEE",
                                    ...
                                    if (INTlist.Contains(dr[l]["D"].ToString().ToUpper()) == false)
                                    {
                                        ...
                                        else
                                        {
                                            if (dr[l].Table.Columns.Contains("TL"))
                                            {
                                                ...
                                                HashSet<string> allowedValues = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
                                                    {
                                                        "A 10", "A 20", "A 40", "A 43", "A 63", "A 64", "A 86", "A 87", "A 96", "A 97",
                                                        "P 10", "P 13", "P 14", "P 15", "P 16", "P 30", "P 32", "P 33", "P 39", "P 96", "P 97",
                                                        "F 29"
                                                    };

                                                // Check if the "FR" value is in the allowed values
                                                if (allowedValues.Contains(dr[l]["FR"].ToString().ToUpper()))
                                                {
                                                    objSTD.TRNDESC = dr[l]["D"].ToString().Replace("'", "''") + " " + dr[l]["TL"].ToString().Replace("'", "''");
                                                }
                                        ...
                                        {
                                            trn_Date = dr[l]["STATEMENT_DATE"].ToString();
                                        }
                                        ...
                                        sql = "Insert into STATEMENT_DETAILS(STATEMENTID,CONTRACTNO,IDCLIENT,PAN,ACCOUNTNO,STATEMENTNO,TRNDATE,POSTDATE,TRNDESC,ACURN,AMOUNT,OC,ORGAMOUNT,AMOUNTSIGN,APPROVAL,FR,P,DOCNO,DE)" +
                                            ...
                                            "'" + objSTD.POSTDATE + "','" + objSTD.TRNDESC + "','" + objSTD.ACURN + "','" + objSTD.AMOUNT + "','" + objSTD.OC + "','" + objSTD.ORGAMOUNT + "','" + objSTD.AMOUNTSIGN + "','" + objSTD.APPROVAL + "','" + objSTD.FR + "','" + objSTD.P + "','" + objSTD.DOCNO +"','" +objSTD.DE+"')";
                                ...
                                if (objSt.SUM_INTEREST != "0.00")
                                {
                                    ...
                                    objSTD.TRNDATE = objSt.STATEMENT_DATE;
                                   // objSTD.TRNDATE = trn_Date;
                                    objSTD.POSTDATE = trn_Date;
                                    objSTD.P = objSt.PAN;
                                    sql = "Insert into STATEMENT_DETAILS(STATEMENTID,CONTRACTNO,IDCLIENT,PAN,ACCOUNTNO,STATEMENTNO,TRNDATE,POSTDATE,TRNDESC,ACURN,AMOUNT,APPROVAL,AMOUNTSIGN,P,DE)" +
                                            " VALUES('" + objSTD.STATEMENTID + "','" + objSTD.CONTRACTNO + "','" + objSTD.IDCLIENT + "','" + objSTD.PAN + "','" + objSTD.ACCOUNTNO + "','" + objSTD.STATEMENTNO + "','" + objSTD.TRNDATE + "'," +
                                            "'" + objSTD.POSTDATE + "','" + objSTD.TRNDESC + "','" + objSTD.ACURN + "','" + objSTD.AMOUNT + "','" + objSTD.APPROVAL + "','" + objSTD.AMOUNTSIGN + "','" + objSTD.P  + "','"+objSTD.DE+"')";
                       ...
                       // else if (dtStatement.Columns[j].ColumnName.ToUpper() == "PROMOTIONALTEXT")
                        //{
                         //   objSt.PROMOTIONALTEXT = dtStatement.Rows[k][j].ToString().Replace("'", "");
                          ...
                          //  {
                           //     objSt.MMFUSDSBALANCE = lines[2].Trim();
                            //}
