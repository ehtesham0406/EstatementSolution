...
using QCash.EStatement.SBL_Prepaid.Reports;
...
namespace StatementGenerator
{
    public partial class EStatementGenerator : Form
    {
        ...
        private void ProcessData()
        {
            ...
            {
                if (dia[fcount].FullName.Contains("SBL"))
                {
                    _bankName = "SBL";
                    _bankCode = "7061";
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        for (int j = 0; j < dtCards.Rows.Count; j++)//dtCards.Rows.Count
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : No PAN present !!!\n : Estatement has not been created for Card# " + dtCards.Rows[j]["EMAIL"].ToString() });
                                    objLW.logTrace(_LogPath, "EStatement.log", System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : No PAN present !!!\n : Estatement has not been created for Card# " + dtCards.Rows[j]["EMAIL"].ToString());
