...
using QCash.EStatement.SONA.Reports;
...
namespace StatementGenerator
{
    public partial class EStatementGenerator : Form
    {
                                    ...
                                    try
                                    {
                                        ...
                                        if ((dtCards.Rows[j]["pan"].ToString()) != vPAN)
                                        {
                                            ...
                                            if (stmdt.Rows.Count > 0)
                                            {
                                               ...
                                               // string Bin = dtCards.Rows[j]["pan"].ToString().Substring(dtCards.Rows[j]["pan"].ToString().Length - 4, 4);
                                                ...
                                                // Set font for paging  
                                                XFont font = new XFont("Calibri", 7);
                                                ...
                                                for (int i = 0; i < document.Pages.Count; ++i)
                                                {
                                                    ...
                                                    XRect layoutRectangle = new XRect(230 /*X*/ , 31.5 /*Y*/ , page.Width /*Width*/ , font.Height /*Height*/ );
                                                    using (XGraphics gfx = XGraphics.FromPdfPage(page))
                                                    {
                                                        //gfx.DrawString("Page " + (i + 1).ToString() + " of " + noPages, font, brush, layoutRectangle, XStringFormats.Center);

                                                        //gfx.DrawString("Page " + (i + 1).ToString() +"of"+ noPages + font, brush, layoutRectangle, XStringFormats.Center);
                                                        gfx.DrawString((i + 1).ToString() + " of " + TPages, font, brush, layoutRectangle, XStringFormats.Center);
        ...
        private void ProcessData()
        {
            ...
            {
                if (dia[fcount].FullName.Contains("SONA"))
                {
                    _bankName = "SONA";
                    _bankCode = "3";
