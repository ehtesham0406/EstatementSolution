...
namespace StatementGenerator.App_Code
{
    public class EStatementDataProvider : IEStatement
    {
                ...
                if (ds.Tables.Count > 0)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                        {
                            ...
                            objEst.EBALANCE_BDT = ds.Tables[0].Rows[i]["EBALANCE_BDT"].ToString();
                            objEst.EBALANCE_USD = ds.Tables[0].Rows[i]["EBALANCE_USD"].ToString();
                            objEst.MIN_AMOUNT_DUE_BDT = ds.Tables[0].Rows[i]["MIN_AMOUNT_DUE_BDT"].ToString();
                            objEst.MIN_AMOUNT_DUE_USD = ds.Tables[0].Rows[i]["MIN_AMOUNT_DUE_USD"].ToString();
                            objEst.PAYMENT_DATE = ds.Tables[0].Rows[i]["PAYMENT_DATE"].ToString();
                            objEst.REWARD_BALANCE = ds.Tables[0].Rows[i]["REWARD_BALANCE"].ToString();
        ...
        public string AddEStatement(EStatementInfo objEst, ref string reply)
        {
            ...
            _reply = objProvider.RunQuery("insert into email_notification values('" + objEst.BANK_CODE + "','" + objEst.IDCLIENT + "','" + objEst.PAN_NUMBER + "','" + objEst.STMDATE + "','" + objEst.MONTH + "','" + objEst.YEAR + "','" + objEst.FILE_LOCATION + "','" + objEst.MAILADDRESS + "','" + objEst.MAILSUBJECT + "','" + objEst.MAILBODY + "','" + objEst.STATUS + "','" + objEst.PAYMENT_DATE + "','" + objEst.EBALANCE_BDT + "','" + objEst.EBALANCE_USD + "','" + objEst.MIN_AMOUNT_DUE_BDT + "','" + objEst.MIN_AMOUNT_DUE_USD + "','" + objEst.REWARD_BALANCE + "')");
            return _reply;
        }
