...
using QCash.EStatement.JBL.Reports;
...
namespace StatementGenerator
{
    public partial class EStatementGenerator : Form
    {
                                ...
                                for (int i = 0; i < objESList.Count; i++)
                                {
                                    if (objESList[i].MAILADDRESS != "")
                                    {
                                        try
                                        {
                                            ...
                                            string htmlTemplate = File.ReadAllText("D:\\XML_For_Email\\MailTemplate\\Template.html");  

                                            // Replace placeholders in the HTML template with dynamic values
                                          //  htmlTemplate = htmlTemplate.Replace("#ID_Value", objESList[i].IDCLIENT);
                                            //htmlTemplate = htmlTemplate.Replace("#Due_Value", Convert.ToDateTime(objESList[i].PAYMENT_DATE).ToString("dd MMM, yyyy"));
                                            htmlTemplate = htmlTemplate.Replace("#Due_Value", Convert.ToDateTime(DateTime.ParseExact(objESList[i].PAYMENT_DATE, "dd/MM/yyyy", CultureInfo.InvariantCulture)).ToString("dd MMM, yyyy"));
                                            htmlTemplate = htmlTemplate.Replace("#Mini_Value_BDT", objESList[i].MIN_AMOUNT_DUE_BDT);
                                            htmlTemplate = htmlTemplate.Replace("#Mini_Value_USD", objESList[i].MIN_AMOUNT_DUE_USD);
                                            htmlTemplate = htmlTemplate.Replace("#End_Value_BDT", objESList[i].EBALANCE_BDT);
                                            htmlTemplate = htmlTemplate.Replace("#End_Value_USD", objESList[i].EBALANCE_USD);
                                            htmlTemplate = htmlTemplate.Replace("#Bonus_Value", objESList[i].REWARD_BALANCE);

                                            // Attach the VISA Logo image (embed it inline without showing it as an attachment)
                                            string visaImagePath = "D:\\XML_For_Email\\MailTemplate\\VISA_Logo.png";
                                            Attachment visaimageAttachment = new Attachment(visaImagePath);
                                            visaimageAttachment.ContentId = "visaLogo";
                                            visaimageAttachment.ContentDisposition.Inline = true;  // Ensure it is treated as inline
                                            visaimageAttachment.ContentDisposition.DispositionType = DispositionTypeNames.Inline; // Not a regular attachment



                                            // Attach the Bank Logo image (embed it inline without showing it as an attachment)
                                            string bankLogoPath = "D:\\XML_For_Email\\MailTemplate\\JBPLC_Logo.png";
                                            Attachment bankLogoAttachment = new Attachment(bankLogoPath);
                                            bankLogoAttachment.ContentId = "bankLogo";
                                            bankLogoAttachment.ContentDisposition.Inline = true;  // Ensure it is treated as inline
                                            bankLogoAttachment.ContentDisposition.DispositionType = DispositionTypeNames.Inline; // Not a regular attachment
                                            ...
                                            mail.IsBodyHtml = true;  // Important to indicate this is an HTML email
                                            mail.To.Add(objESList[i].MAILADDRESS.Trim());
                                            System.Net.Mail.Attachment attachment;
                                            attachment = new System.Net.Mail.Attachment(objESList[i].FILE_LOCATION);
                                            mail.Attachments.Add(attachment);
                                            mail.Attachments.Add(bankLogoAttachment);
                                            mail.Attachments.Add(visaimageAttachment);
                                ...
                                if (IsValid(dtCards.Rows[j]["EMAIL"].ToString().Trim()))
                                {
                                    try
                                    {
                                        ...
                                        if (stmdt.Rows.Count > 0)
                                        {
                                            ...
                                            System.IO.Stream st = objst.ExportToStream(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat);
                                            ...
                                            objEst.MAILSUBJECT = "Your Jamuna Bank Credit Card E-Statement is Ready";
                                            ...
                                            //CHNG
                                            cusdt = new DataTable();
                                            //cusdt = objProvider.ReturnData("Select    ACURN,EBALANCE,IDCLIENT,MIN_AMOUNT_DUE,EBALANCE_FORBONUS,PAYMENT_DATE from statement_DUAL where CONTRACTNO='" + dtCards.Rows[j]["CONTRACTNO"].ToString() + "' ORDER BY SL,[AutoID]", ref reply).Tables[0];

                                            cusdt = objProvider.ReturnData(@"
                                                            WITH CTE AS (
                                                                SELECT ACURN, EBALANCE, IDCLIENT, MIN_AMOUNT_DUE, 
                                                                       EBALANCE_FORBONUS, PAYMENT_DATE,
                                                                       ROW_NUMBER() OVER (PARTITION BY ACURN ORDER BY SL, [AutoID] DESC) AS RowNum
                                                                FROM statement_DUAL
                                                                WHERE CONTRACTNO = '" + dtCards.Rows[j]["CONTRACTNO"].ToString() + @"'
                                                            ) 
                                                            SELECT ACURN, EBALANCE, IDCLIENT, MIN_AMOUNT_DUE, 
                                                                   EBALANCE_FORBONUS, PAYMENT_DATE
                                                            FROM CTE WHERE RowNum = 1", ref reply).Tables[0];



                                            foreach (DataRow row in cusdt.Rows)
                                            {
                                                string currency = row["ACURN"].ToString();
                                                decimal parsedBalance;

                                                if (currency == "BDT")
                                                {
                                                    //objEst.EBALANCE_BDT = row["EBALANCE"].ToString();
                                                    string ebal = row["EBALANCE"].ToString(); // Assign BDT value
                                                    if (decimal.TryParse(ebal, out parsedBalance))
                                                    {
                                                        if (parsedBalance < 0)
                                                        {
                                                            objEst.EBALANCE_BDT = Math.Abs(parsedBalance).ToString(); // Convert to positive value
                                                        }
                                                        else
                                                        {
                                                            // If balance is positive, add " CR" at the end
                                                            objEst.EBALANCE_BDT = parsedBalance.ToString() + " CR";
                                                        }
                                                    }

                                                    objEst.MIN_AMOUNT_DUE_BDT = row["MIN_AMOUNT_DUE"].ToString();// Assign BDT value
                                                }
                                                else if (currency == "USD")
                                                {
                                                    //objEst.EBALANCE_USD = row["EBALANCE"].ToString();
                                                    string ebal = row["EBALANCE"].ToString(); // Assign BDT value
                                                    if (decimal.TryParse(ebal, out parsedBalance))
                                                    {
                                                        if (parsedBalance < 0)
                                                        {
                                                            objEst.EBALANCE_USD = Math.Abs(parsedBalance).ToString(); // Convert to positive value
                                                        }
                                                        else
                                                        {
                                                            // If balance is positive, add " CR" at the end
                                                            objEst.EBALANCE_USD = parsedBalance.ToString() + " CR";
                                                        }
                                                    }

                                                    objEst.MIN_AMOUNT_DUE_USD = row["MIN_AMOUNT_DUE"].ToString();
                                                }

                                                objEst.IDCLIENT = row["IDCLIENT"].ToString();
                                                objEst.REWARD_BALANCE = row["EBALANCE_FORBONUS"].ToString();

                                                objEst.PAYMENT_DATE = row["PAYMENT_DATE"].ToString();
                                            ...
                                            //  objEst.REWARD_BALANCE = stmdt.Rows[0]["EBALANCE_FORBONUS"].ToString();
                        ...
                        for (int j = 0; j < dtCards.Rows.Count; j++)//dtCards.Rows.Count
                        {
                            if (dtCards.Rows[j]["EMAIL"].ToString().Trim() != "")
                            {
                                if (IsValid(dtCards.Rows[j]["EMAIL"].ToString().Trim()))
                                {
                                    ...
                                    {
                                        stmdt = new DataTable();
                                        stmdt = objProvider.ReturnData("select * from statement_DUAL where pan='" + dtCards.Rows[j]["pan"].ToString() + "'", ref reply).Tables[0];
                                        if (stmdt.Rows.Count > 0)
                                        {
                                            EStatement objst = new EStatement();
                                            objst.SetDataSource(stmdt);
                                            ...
                                            objst.ExportToDisk(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, filePath + "\\" + fileName);
                                            ...
                                            //CHNG
                                            foreach (DataRow row in stmdt.Rows)
                                            {
                                                string currency = row["ACURN"].ToString();

                                                if (currency == "BDT")
                                                {
                                                    objEst.EBALANCE_BDT = row["EBALANCE"].ToString();  // Assign BDT value
                                                    objEst.MIN_AMOUNT_DUE_BDT = row["MIN_AMOUNT_DUE"].ToString();  // Assign BDT value
                                                }
                                                else if (currency == "USD")
                                                {
                                                    objEst.EBALANCE_USD = row["EBALANCE"].ToString();  // Assign USD value
                                                    objEst.MIN_AMOUNT_DUE_USD = row["MIN_AMOUNT_DUE"].ToString();  // Assign USD value
                                                }
                                            }
                                            objEst.PAYMENT_DATE = stmdt.Rows[0]["PAYMENT_DATE"].ToString();
                                            objEst.REWARD_BALANCE = stmdt.Rows[0]["EBALANCE_FORBONUS"].ToString();
        ...
        private void ProcessData()
        {
            ...
            {
                if (dia[fcount].FullName.Contains("JBL"))
                {
                    _bankName = "JBL";
                    _bankCode = "9";
                        ...
                        if (dsXML.Tables.Count > 0)
                        {
                            ...
                            for (int i = 0; i < dsXML.Tables.Count; i++)
                            {
                                ...
                                else if (dsXML.Tables[i].TableName == "BonusContrAcc")
                                {
                                    ...
                                    txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : CardHolder Bonus Info data Saved from XML. " + reply });
                                    objLW.logTrace(_LogPath, "EStatement.log", " : CardHolder Bonus Info data Saved from XML. " + reply);
                                }
                                else if (dsXML.Tables[i].TableName == "AccumIntAcc")
                                {
                                    reply = GetAccumIntAcc(dsXML.Tables[i]);
                                    txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : CardHolder Suspense Info data Saved from XML. " + reply });
                                    objLW.logTrace(_LogPath, "EStatement.log", " : CardHolder Suspense Info data Saved from XML. " + reply);
                                }
                            }
                            if (reply == "Success")
                            {
                                            ...
                                            for (int x = 0; x < dtAcI.Rows.Count; x++)
                                            {
                                                ...
                                                if (objSTD.CONTRACTNO == dtAcI.Rows[x][1].ToString())
                                                {
                                                    if (dtAcI.Rows[x][0].ToString() != "0.00")
                                                    {
                                                        ...
                                                        objSTD.AMOUNT = "-" + dtAcI.Rows[x][0].ToString();//.PadLeft(objSt.SUM_INTEREST.Length+1,'-');
                                                        objSTD.TRNDATE = objSTD.TRNDATE;
                                                        objSTD.POSTDATE = objSTD.TRNDATE;
                                ...
                                for (int l = 0; l < dr.Length; l++)
                                {
                                    ...
                                    if (INTlist.Contains(dr[l]["D"].ToString().ToUpper()) == false)
                                    {
                                        ...
                                        if (dr[l].Table.Columns.Contains("APPROVAL"))
                                        {
                                            ...
                                            {
                                                objSTD.TRNDATE = dr[l]["OD"].ToString();
                                                if (!string.IsNullOrEmpty(objSTD.TRNDATE)) //CHNG
                                                {
                                                    string[] tddate = objSTD.TRNDATE.Split('/');
