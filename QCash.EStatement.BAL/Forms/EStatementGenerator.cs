...
using QCash.EStatement.BAL.Reports;
using QCash.EStatement.BAL.App_Code;
using System.Globalization;
using QCash.EStatement.BAL;
...
namespace StatementGenerator
{
    public partial class EStatementGenerator : Form
    {
        ...
        private void ProcessData()
        {
            ...
            {
                 if (dia[fcount].FullName.Contains("BAL"))
                {
                    _bankName = "BAL";
                    _bankCode = "1";
        ...
        private DataSet getDataFromXML_(string _filename)
        {
            try
            {
                // Check if the file is encrypted
                if (_filename.EndsWith(".enc", StringComparison.OrdinalIgnoreCase))
                {
                    // Decrypt the file content
                    string decryptedXml = XmlEncryptor.DecryptXml(_filename);

                    // Load DataSet from decrypted XML string
                    DataSet ds = new DataSet();
                    using (System.IO.StringReader sr = new System.IO.StringReader(decryptedXml))
                    {
                        ds.ReadXml(sr);
                    }
                    return ds;
