...
using QCash.EStatement.SJIBL;
using CrystalDecisions.CrystalReports.Engine;
using FlexiStar.Utilities.EncryptionEngine;
using QCash.EStatement.SJIBL.Reports;
...
namespace StatementGenerator
{
    public partial class EStatementGenerator : Form
    {
        ...
        public static string ACCUM_AMOUNT_GLOBAL;
        //
                                        ...
                                        //{
                                          ...
                                          //  vPAN = dtCards.Rows[j]["pan"].ToString();
                                            if (stmdt.Rows.Count > 0)
                                            {
                                                ...
                                                //fileName = _fiid + "_" + dtCards.Rows[j]["pan"].ToString().Substring(0, 6) + "_" + stmdt.Rows[0]["Statement_Date"].ToString().Replace('/', '-') + '_' + pdfCount + ".pdf"; ;
                                                //objst.ExportToDisk(ExportFormatType.PortableDocFormat, filePath + "\\" + fileName);

                                                EStatement objst = new EStatement();
                                                objst.SetDataSource(stmdt);
                                               
                                                //fileName = _fiid + "_" + dtCards.Rows[j]["pan"].ToString().Substring(0, 6) + "_" + stmdt.Rows[0]["Statement_Date"].ToString().Replace('/', '-') + '_' + stmdt.Rows[0]["IDCLIENT"] + '_' + pdfCount + ".pdf";
        ...
        private void ProcessData()
        {
            ...
            {
                if (dia[fcount].FullName.Contains("SJIBL"))
                {
                    _bankName = "SJIBL";
                    _bankCode = "1";
                ...
                else if (dia[fcount].FullName.Contains("SJBL"))
                {
                    _bankName = "SJIBL";
                    _bankCode = "1";
                    _XMLSourcePath = dia[fcount].FullName;
                ...
                else if (dia[fcount].FullName.Contains("MTBL"))
                {
                    _bankName = "MTBL";
                    _bankCode = "21";
                    _XMLSourcePath = dia[fcount].FullName;
                        ...
                        if (dsXML.Tables.Count > 0)
                        {
                            ...
                            for (int i = 0; i < dsXML.Tables.Count; i++)
                            {
                                ...
                                else if (dsXML.Tables[i].TableName == "AccumIntAcc")
                                {
                                    ...
                                    objLW.logTrace(_LogPath, "EStatement.log", " : CardHolder Suspense acount Info data Saved from XML. " + reply);
                                }
                            }
                            if (reply == "Success")
                            {
            ...
            try
            {
                ...
                for (int k = 0; k < dtStatement.Rows.Count; k++)
                {
                    ...
                    sql = "Insert into Statement(BANK_CODE,STATEMENTNO,ADDRESS,CARD_LIST,CITY,COUNTRY,EMAIL," +
                          "STARTDATE,ENDDATE,CLIENT,CONTRACTNO,IDCLIENT,FAX,JOBTITLE,MAIN_CARD,MOBILE," +
                          ...
                          "'" + objSt.STARTDATE + "','" + objSt.ENDDATE + "','" + objSt.CLIENT + "','" + objSt.CONTRACTNO + "','" + objSt.IDCLIENT + "','" + objSt.FAX + "','" + objSt.JOBTITLE + "','" + objSt.MAIN_CARD + "','" + objSt.MOBILE + "'," +
        ...
        private void GenerateStatementInfo(DataSet dsStatement,string BankName, ref string errMsg)
        {
            ...
            try
            {
                ...
                //DataSet dsBDT = objProvider.ReturnData("select * from Qry_Card_Account where Curr='TK'", ref reply);
        ...
        public void GetAccumAmount(string contractNo)
        {
            string reply = string.Empty;
            DataSet dsAcI = objProvider.ReturnData("select * from ACCUM_BODY_VW where contractno='" + contractNo + "'", ref reply);
            
            ACCUM_AMOUNT_GLOBAL = null;
            // Check if the dataset has data
            if (dsAcI != null && dsAcI.Tables.Count > 0 && dsAcI.Tables[0].Rows.Count > 0)
            {
                // Store the value in the global static variable
                ACCUM_AMOUNT_GLOBAL = (dsAcI.Tables[0].Rows[0]["ACCUM_INT_AMOUNT"]).ToString();
            }
            else
            {
                // If no data is found or there is an error, assign 0.00
                ACCUM_AMOUNT_GLOBAL = "0.00";  
            }
        }
            ...
            for (int k = 0; k < dtStatement.Rows.Count; k++)
            {
                ...
                try
                {
                    ...
                    sql = "Insert into STATEMENT_INFO(STATEMENTID,BANK_CODE,CONTRACTNO,IDCLIENT,PAN,TITLE,CLIENTNAME,STATEMENTNO,ADDRESS,CITY,ZIP,COUNTRY," +
                        ...
                        "MIN_AMOUNT_DUE,CASH_LIMIT,CRD_LIMIT,STM_MSG,STATUS,JOBTITLE) VALUES('" + objSt.STATEMENTID + "'," +
                        ...
                        "'" + objSt.CRD_LIMIT + "','" + objSt.STM_MSG + "','" + objSt.STATUS + "','" + objSt.JOBTITLE + "')";
                    ...
                    if (dtOperation != null && dtOperation.Columns.Contains("ACCOUNT"))
                    {
                        if (dtOperation.Rows.Count > 0)
                        {
                            ...
                            if (dr.Length > 0)
                            {
                                ...
                                {
                                    #region setting properties values
                                    //List<string> INTlist = new List<string>() { "PROFIT ON FEES & CHARGES", "PROFIT ON PROFIT", "PROFIT ON ATM TRANSACTION", "PROFIT ON POS TRANSACTION", "PROFIT ON CARD CHEQUE", "CHARGE PROFIT FOR 1", "CHARGE PROFIT FOR 2", "CHARGE PROFIT FOR 3", "CHARGE PROFIT FOR 4", "CHARGE PROFIT FOR 5", "CHARGE PROFIT FOR 6", "CHARGE PROFIT FOR 7", "CHARGE PROFIT FOR 8", "CHARGE PROFIT FOR 9", "CHARGE PROFIT FOR 10", "CHARGE PROFIT FOR 11", "CHARGE PROFIT FOR 0 OPERATIONS GROUP", "CHARGE PROFIT FOR 1 OPERATIONS GROUP", "CHARGE PROFIT FOR 2 OPERATIONS GROUP", "CHARGE PROFIT FOR 3 OPERATIONS GROUP", "CHARGE PROFIT FOR 4 OPERATIONS GROUP", "CHARGE PROFIT FOR 5 OPERATIONS GROUP", "CHARGE PROFIT FOR 6 OPERATIONS GROUP", "CHARGE PROFIT FOR 7 OPERATIONS GROUP", "PROFIT ON FUND TRANSFER", "PROFIT ON BALANCE TRANSFER", "PROFIT ON EMI", "PROFIT ON FT", "PROFIT ON BT" };
                                   // List<string> INTlist = new List<string>() { "UZRAT OF WAKALAH", "PROFIT ON FEES & CHARGES", "PROFIT ON PROFIT", "PROFIT ON ATM TRANSACTION", "PROFIT ON POS TRANSACTION", "PROFIT ON CARD CHEQUE", "PROFIT ON EMI", "PROFIT ON MONTHLY EMI", "PROFIT ON CANCELLED EMI", "PROFIT ON FUND TRANSFER", "PROFIT ON BALANCE TRANSFER", "PROFIT ON FT", "PROFIT ON BT", "CHARGE PROFIT FOR 1", "CHARGE PROFIT FOR 2", "CHARGE PROFIT FOR 3", "CHARGE PROFIT FOR 4", "CHARGE PROFIT FOR 5", "CHARGE PROFIT FOR 6", "CHARGE PROFIT FOR 7", "CHARGE PROFIT FOR 8", "CHARGE PROFIT FOR 9", "CHARGE PROFIT FOR 10", "CHARGE PROFIT FOR 11", "CHARGE PROFIT FOR 0 OPERATIONS GROUP", "CHARGE PROFIT FOR 1 OPERATIONS GROUP", "CHARGE PROFIT FOR 2 OPERATIONS GROUP", "CHARGE PROFIT FOR 3 OPERATIONS GROUP", "CHARGE PROFIT FOR 4 OPERATIONS GROUP", "CHARGE PROFIT FOR 5 OPERATIONS GROUP", "CHARGE PROFIT FOR 6 OPERATIONS GROUP", "CHARGE PROFIT FOR 7 OPERATIONS GROUP", "CHARGE INTEREST FOR 0 OPERATIONS GROUP", "CHARGE OF A DEBT FOR CREDIT OVERDRAFTING" };

                                    List<string> INTlist = new List<string>() { "UZRAT OF WAKALAH", "PROFIT ON FEES & CHARGES", "PROFIT ON OTHERS","PROFIT ON PROFIT", "PROFIT ON ATM TRANSACTION", "PROFIT ON POS TRANSACTION", "PROFIT ON CARD CHEQUE", "CHARGE PROFIT FOR 0", "CHARGE PROFIT FOR 1", "CHARGE PROFIT FOR 2", "CHARGE PROFIT FOR 3", "CHARGE PROFIT FOR 4", "CHARGE PROFIT FOR 5", "CHARGE PROFIT FOR 6", "CHARGE PROFIT FOR 7", "CHARGE PROFIT FOR 8", "CHARGE PROFIT FOR 9", "CHARGE PROFIT FOR 10", "CHARGE PROFIT FOR 11", "CHARGE PROFIT FOR 0 OPERATIONS GROUP", "CHARGE PROFIT FOR 1 OPERATIONS GROUP", "CHARGE PROFIT FOR 2 OPERATIONS GROUP", "CHARGE PROFIT FOR 3 OPERATIONS GROUP", "CHARGE PROFIT FOR 4 OPERATIONS GROUP", "CHARGE PROFIT FOR 5 OPERATIONS GROUP", "CHARGE PROFIT FOR 6 OPERATIONS GROUP", "CHARGE PROFIT FOR 7 OPERATIONS GROUP", "PROFIT ON FUND TRANSFER", "PROFIT ON BALANCE TRANSFER", "PROFIT ON EMI","PROFIT ON FT", "PROFIT ON BT", "PROFIT ON BANK POS TRANSACTION", "PROFIT ON BPOS TRANSACTION",
                                        "CHARGE PROFIT FOR PROFIT OPERATIONS", "CHARGE PROFIT FOR POS OPERATIONS", "CHARGE PROFIT FOR ATM OPERATIONS", "LATE PAYMENT CHARGE FOR GROUP 1", "LATE PAYMENT CHARGE FOR GROUP 2", "LATE PAYMENT CHARGE FOR GROUP 3", "CHARGE OF A DEBT FOR CREDIT OVERDRAFTING", "PROFIT ON SERVICE FEE", "PROFIT ON PREVIOUS BALANCE","REVOLVING PROFIT CHARGE" ,"CHARGE INTEREST FOR 0", "CHARGE INTEREST FOR 1", "CHARGE INTEREST FOR 2", "CHARGE INTEREST FOR 3", "CHARGE INTEREST FOR 4", "CHARGE INTEREST FOR 5", "CHARGE PROFIT FOR 6", "CHARGE INTEREST FOR 7", "CHARGE INTEREST FOR 8", "CHARGE INTEREST FOR 9", "CHARGE INTEREST FOR 10", "CHARGE INTEREST FOR 11", "CHARGE INTEREST FOR 0 OPERATIONS GROUP", "CHARGE INTEREST FOR 1 OPERATIONS GROUP", "CHARGE INTEREST FOR 2 OPERATIONS GROUP", "CHARGE INTEREST FOR 3 OPERATIONS GROUP", "CHARGE INTEREST FOR 4 OPERATIONS GROUP", "CHARGE PROFIT FOR 5 OPERATIONS GROUP", "CHARGE INTEREST FOR 6 OPERATIONS GROUP", 
                                        "CHARGE INTEREST FOR 7 OPERATIONS GROUP", "PROFIT ON FUND TRANSFER","CHARGE INTEREST FOR INTEREST OPERATIONS", "CHARGE INTEREST FOR POS OPERATIONS", "CHARGE INTEREST FOR ATM OPERATIONS", "LATE PAYMENT CHARGE FOR GROUP 1", "LATE PAYMENT CHARGE FOR GROUP 2", "LATE PAYMENT CHARGE FOR GROUP 3", "CHARGE OF A DEBT FOR CREDIT OVERDRAFTING", "INTEREST ON SERVICE FEE", "INTEREST ON PREVIOUS BALANCE","REVOLVING INTEREST CHARGE"};
                                    ...
                                    if (INTlist.Contains(dr[l]["D"].ToString().ToUpper()) == false)
                                    {
                                        ...
                                        #region Monthly EMI ,TRANSFER TO EMI,EMI CANCELLED,EMI
                                        ...
                                        if ((dr[l]["D"].ToString().ToUpper().Contains("MONTHLY EMI")) || (dr[l]["D"].ToString().ToUpper().Contains("TRANSFER TO EMI")) || (dr[l]["D"].ToString().ToUpper().Contains("EMI CANCELLED")))
                                        {
                                            if (dr[l].Table.Columns.Contains("FR"))
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    {
                                                        // objSTD.TRNDESC = data.ToString().Replace("\n", "");
                                        ...
                                        #endregion Monthly EMI ,TRANSFER TO EMI,EMI CANCELLED,EMI
                                               ...
                                               /* if (dr[l]["D"].ToString().ToUpper().Contains("PURCHASE"))
                                                {
                                                    if (dr[l]["D"].ToString().Trim().Length > 8)
                                                    {
                                                        ...
                                                        objSTD.TRNDESC = (dr[l]["D"].ToString().ToUpper().Replace("PURCHASE", "")).Trim() + " " + dr[l]["TL"].ToString().Replace("'", "''") + " " + "(" + objSTD.OC + " " + dr[l]["OA"].ToString() + ")";
                                                    }
                                                    else
                                                    {
                                                        objSTD.TRNDESC = (dr[l]["D"].ToString().ToUpper().Replace("PURCHASE", "")).Trim() + dr[l]["TL"].ToString().Replace("'", "''") + " " + "(" + objSTD.OC + " " + dr[l]["OA"].ToString() + ")";
                                        ...
                                        #endregion PAYMENT CASH DEPOSIT
                                        ...
                                        #region INTEREST/PROFIT CHARGES TRANSACTION

                                        if ((dr[l]["D"].ToString().ToUpper().Trim() == ("INTEREST CHARGES")) || (dr[l]["D"].ToString().ToUpper().Trim() == ("PROFIT CHARGES")))
                                        {

                                            objSTD.TRNDESC = "PROFIT CHARGE";
                                        }
                                        ...
                                        if ((objSTD.TRNDESC.ToUpper().Contains("INTEREST")))
                                        {
                                            //objSTD.TRNDESC = dr[l]["D"].ToString().Replace("Interest", "Profit");
                                            objSTD.TRNDESC = dr[l]["D"].ToString().ToUpper().Replace("INTEREST", "PROFIT");
                                        }
                                ...
                                GetAccumAmount(_contractNo);

                                //New View add
                                if (objSt.SUM_INTEREST != "0.00" && ACCUM_AMOUNT_GLOBAL=="0.00")
                                {
                                    StatementDetails objSTD = new StatementDetails();
                                    objSTD.STATEMENTID = objSt.STATEMENTID;
                                    objSTD.CONTRACTNO = objSt.CONTRACTNO;
                                    ...
                                    objSTD.TRNDESC = "UZRAT OF WAKALAH REALIZE";
                                    //objSTD.TRNDESC = "UZRAT OF WAKALAH REALIZE";
                                                ...
                                                for (int x = 0; x < dtAcI.Rows.Count; x++)
                                                {
                                                    ...
                                                    if (objSTD.CONTRACTNO == dtAcI.Rows[x][1].ToString())
                                                    {
                                                        if (dtAcI.Rows[x][0].ToString() != "0.00")
                                                        {
                                                            ...
                                                            objSTD.ACCOUNTNO = objSt.ACCOUNTNO;
                                                            objSTD.ACURN = objSt.ACURN;
                                                            objSTD.TRNDESC = "UZRAT OF WAKALAH/KHIDMAH REALIZE";
                                                            ...
                                                            dt = objProvider.ReturnData("select AMOUNT from STATEMENT_DETAILS WHERE STATEMENTNO= '" + objSTD.STATEMENTNO + "' AND CONTRACTNO= '" + objSTD.CONTRACTNO + "' AND TRNDESC= 'UZRAT OF WAKALAH REALIZE' ", ref reply).Tables[0];
                            ...
                            #region If Customer have no Transaction but have Accum Interest
        ...
        {
            if (txtEmailSubject.Text.Length > 999)
                MessageBox.Show("Your mail subject exceeded maximum character limit! Mail subject must be within 1000 character.");
        }

        private void txtEmailBody_TextChanged(object sender, EventArgs e)
        {
            if (txtEmailBody.Text.Length > 999)
                MessageBox.Show("Your mail body exceeded maximum character limit! Mail body length must be within 1000 character.");
        }

        private void txtStmMsg_TextChanged(object sender, EventArgs e)
        {
            if (txtStmMsg.Text.Length > 500)
                MessageBox.Show("Your text message exceeded maximum character limit! Message length must be within 500 character.");
        }
