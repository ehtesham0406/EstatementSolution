...
using QCash.EStatement.BCBL.Reports;
    ...
    public partial class EStatementGenerator : Form
    {
                                    ...
                                    if (objESList[i].MAILADDRESS != "")
                                    {
                                        try
                                        {
                                           ...
                                           // mail.From = new MailAddress(objSmtpList[0].From_Address);
                                            mail.From = new MailAddress(objSmtpList[0].From_Address, "BCBL Card Division");
        ...
        void btnGenerate_Click(object sender, EventArgs e)
        {
            ...
            if (dtCardbdt.Rows.Count > 0)
            {
                ...
                objLW.logTrace(_LogPath, "EStatement.log", System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : Processing Estatement BDT.");//Processing Estatement BDT.
        ...
        /*    DataTable dtCardusd = new DataTable();
            dtCardusd = objProvider.ReturnData("select * from Qry_Card_Account where Curr='USD'", ref reply).Tables[0]; // where Curr='USD'
            if (dtCardusd != null)
            {
                                        ...
                                        stmdt = objProvider.ReturnData("select * from statement_DUAL where CONTRACTNO='" + dtCards.Rows[j]["CONTRACTNO"].ToString() + "' ORDER BY SL,[AutoID]", ref reply).Tables[0];
                                            ...
                                            if (stmdt.Rows.Count > 0)
                                            {
                                                ...
                                                for (int i = 0; i < document.Pages.Count; ++i)
                                                {
                                                    ...
                                                    XRect layoutRectangle = new XRect(250 /*X*/ , 800 /*Y*/ , page.Width /*Width*/ , font.Height /*Height*/ );
        ...
        private void ProcessData()
        {
            ...
            {
                if (dia[fcount].FullName.Contains("BCBL"))
                {
                    _bankName = "BCBL";
                    _bankCode = "3";
                        ...
                        if (dsXML.Tables.Count > 0)
                        {
                            ...
                            for (int i = 0; i < dsXML.Tables.Count; i++)
                            {
                                ...
                                else if (dsXML.Tables[i].TableName == "AccumIntAcc")
                                {
                                    ...
                                    txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:tt") + " : CardHolder Card Info data Saved from XML. " + reply });
                                    objLW.logTrace(_LogPath, "EStatement.log", " : CardHolder AccumInt Info data Saved from XML. " + reply);
                                }
                            }
                            if (reply == "Success")
                            {
